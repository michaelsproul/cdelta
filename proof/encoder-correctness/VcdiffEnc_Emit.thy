...
lemma emit_copy'_small_addr_byte_success_heap_bytes2_frame:
    ...
    apply runs_to_vcg
      apply (rule runs_to_weaken[
        OF write_byte'_heap_bytes_append_next_typing_preserves2_near_ptr])
          ...
          apply (rule inst_byte_ptr)
         apply (rule inst_byte_dist)
        apply (rule inst_byte_disj1)
       apply (rule inst_byte_disj2)
     apply clarsimp
     apply runs_to_vcg
     apply (rule runs_to_weaken[
       OF emit_address'_success_byte_heap_bytes_append_preserves2_near_ptr
        [of m "sections_t_C.addr_pos_C sec" addr_cap _
           addr_buf out1_n out1 out2_n out2]])
          apply (rule mode_ge)
         apply (rule addr_byte_fits)
        apply (simp add: addr_byte_ptr)
       apply (rule addr_byte_dist)
      apply (rule addr_byte_disj1)
     apply (rule addr_byte_disj2)
    apply clarsimp
    apply runs_to_vcg
    apply (rule runs_to_weaken[
      OF cache_update'_preserves_heap_bytes3
       [of _ _ out1 out1_n out2 out2_n inst 0]])
     using near_ptr_lt apply simp
    by (auto simp: sections_result_def)
qed

lemma emit_copy'_small_addr_varint_success_heap_bytes2_frame:
  assumes bm: "best_mode' copy_addr here s = Some m"
      and sz_ge: "(4 :: 32 word) \<le> copy_len"
      and sz_le: "copy_len \<le> (18 :: 32 word)"
      and mode_lt: "mode_t_C.mode_C m < (6 :: 32 word)"
      ...
      and addr_varint_disj1: "\<forall>k < out1_n. \<forall>i.
        i < an \<longrightarrow>
        out1 +\<^sub>p int k \<noteq> addr_buf +\<^sub>p uint (sections_t_C.addr_pos_C sec + i)"
      and addr_varint_disj2: "\<forall>k < out2_n. \<forall>i.
        i < an \<longrightarrow>
        out2 +\<^sub>p int k \<noteq> addr_buf +\<^sub>p uint (sections_t_C.addr_pos_C sec + i)"
  shows "emit_copy' sec inst inst_cap addr_buf addr_cap copy_addr here copy_len
            \<bullet> s
         \<lbrace> \<lambda>r t. \<exists>sec'.
              r = Result sec' \<and>
    ...
    apply runs_to_vcg
     ...
     apply (rule runs_to_weaken)
      apply (rule emit_address'_success_varint_heap_bytes_append_preserves2_near_ptr
        [of m _ an addr_cap "sections_t_C.addr_pos_C sec" addr_buf
           out1_n out1 out2_n out2])
             apply (rule mode_lt)
            subgoal for t
      ...
      apply (rule addr_varint_disj1)
     apply (rule addr_varint_disj2)
    apply clarsimp
    apply runs_to_vcg
    apply (rule runs_to_weaken[
      OF cache_update'_preserves_heap_bytes3
...
lemma emit_copy'_large_addr_byte_success_heap_bytes2_frame:
  assumes bm: "best_mode' copy_addr here s = Some m"
      and sz_large:
        "\<not> ((4 :: 32 word) \<le> copy_len \<and> copy_len \<le> (18 :: 32 word))"
      and size: "varint_size' copy_len s = Some sn"
      ...
      and inst_varint_disj1: "\<forall>k < out1_n. \<forall>i.
        i < sn \<longrightarrow>
        out1 +\<^sub>p int k \<noteq> inst +\<^sub>p uint (sections_t_C.inst_pos_C sec + 1 + i)"
      and inst_varint_disj2: "\<forall>k < out2_n. \<forall>i.
        i < sn \<longrightarrow>
        out2 +\<^sub>p int k \<noteq> inst +\<^sub>p uint (sections_t_C.inst_pos_C sec + 1 + i)"
      and addr_byte_fits: "sections_t_C.addr_pos_C sec < addr_cap"
      and addr_byte_ptr:
        "ptr_valid (heap_typing s)
          (addr_buf +\<^sub>p uint (sections_t_C.addr_pos_C sec))"
    ...
    apply runs_to_vcg
     ...
     apply (rule runs_to_weaken)
      apply (rule write_varint'_success_heap_bytes_append_wordpos_preserves2_near_ptr
        [of copy_len _ sn inst_cap "sections_t_C.inst_pos_C sec + 1"
           inst out1_n out1 out2_n out2])
             subgoal for t
         ...
         apply (rule inst_varint_prefix_disj)
        apply (rule inst_varint_no_overflow)
       apply (rule inst_varint_disj1)
      apply (rule inst_varint_disj2)
    apply clarsimp
    apply runs_to_vcg
    apply (rule runs_to_weaken[
      OF emit_address'_success_byte_heap_bytes_append_preserves2_near_ptr
...
lemma emit_copy'_large_addr_varint_success_heap_bytes2_frame:
  assumes bm: "best_mode' copy_addr here s = Some m"
      and sz_large:
        "\<not> ((4 :: 32 word) \<le> copy_len \<and> copy_len \<le> (18 :: 32 word))"
      and size: "varint_size' copy_len s = Some sn"
...
lemma flush_pending'_len_four_enc_sections_cache_inv_cases:
  assumes inv:
        "enc_sections_inv s data inst addr sec src_seg tgt_len
          data_bytes inst_bytes addr_bytes target c_out"
      and abs: "enc_cache_abs s c_out"
       ...
       \<lbrace> \<lambda>r t.
          ((\<exists>sec'.
           ...
           (\<exists>sec'.
            ...
            enc_cache_wf c_out)) \<and>
          heap_typing t = heap_typing s \<rbrace>"
    by (rule flush_pending'_len_four_enc_sections_cache_inv_cases[
        OF inv abs cache_wf pending_valid size target_room sec_ok
           inst_byte_fits inst_byte_ptr inst_byte_dist
           inst_byte_data_disj inst_byte_addr_disj inst_byte_pending_disj
           inst_varint_fits inst_varint_valid inst_varint_inj
           inst_varint_prefix_disj inst_varint_no_overflow
           ...
           data_byte_inst_disj data_byte_addr_disj])
  show ?thesis
    apply (rule runs_to_weaken[OF cases])
    by auto
qed
  ...
  shows "flush_pending' sec data data_cap inst inst_cap pending len \<bullet> s
           \<lbrace> \<lambda>r t.
              (\<exists>sec' data_bytes' inst_bytes' addr_bytes'.
                ...
                enc_cache_wf c_out) \<and>
              heap_typing t = heap_typing s \<rbrace>"
  by (rule flush_pending'_short_enc_sections_cache_inv_step[
      OF inv abs cache_wf len_le pending_valid size target_room sec_ok
         inst_byte_fits inst_byte_ptr inst_byte_dist inst_byte_data_disj
         inst_byte_addr_disj inst_byte_pending_disj inst_varint_fits
         inst_varint_valid inst_varint_inj inst_varint_prefix_disj
         inst_varint_no_overflow inst_varint_data_disj inst_varint_addr_disj
         data_fits data_valid data_pending_disj data_inj data_prefix_disj
         data_no_overflow data_inst_disj data_addr_disj data_byte_fits
         data_byte_ptr data_byte_dist data_byte_inst_disj data_byte_addr_disj])
...
lemma flush_pending_outer_after_scan_preserves_inv_frame:
  fixes P :: "lifted_globals \<Rightarrow> bool"
  assumes inv:
        "flush_pending_outer_loop_inv src_len s0 data inst addr pending len
          spec_st add_start i sec_cur t"
      and P_t: "P t"
          ...
          \<exists>sec'.
            ...
            enc_sections_state_rel u data inst addr sec'
              (flush_pending_outer_run_state src_len s0 pending
                add_start i j b loop_st) \<and>
            P u \<rbrace>"
    by (rule branch[OF rel eq True])
  show ?thesis
    using True
    apply simp
    apply (rule runs_to_weaken[OF branch_run])
    apply clarsimp
    subgoal for u sec'
    proof -
      assume sec_ok': "sections_t_C.err_C sec' = ENC_OK"
...
next
  ...
  have inv':
    ...
    by (rule flush_pending_outer_loop_inv_short_step[
      OF inv i_lt_j j_le_len run_end False])
  show ?thesis
    using False i_lt_j j_le_len inv' P_t by simp
qed

lemma flush_pending_outer_scan_tail_preserves_inv_frame:
  fixes P :: "lifted_globals \<Rightarrow> bool"
  assumes inv:
        "flush_pending_outer_loop_inv src_len s0 data inst addr pending len
          spec_st add_start i sec_cur t"
...
lemma flush_pending_outer_scan_tail_preserves_inv_do_frame:
  fixes P :: "lifted_globals \<Rightarrow> bool"
  assumes inv:
        "flush_pending_outer_loop_inv src_len s0 data inst addr pending len
          spec_st add_start i sec_cur t"
...
lemma flush_pending_outer_body_preserves_inv_frame:
  fixes P :: "lifted_globals \<Rightarrow> bool"
  assumes inv:
        "flush_pending_outer_loop_inv src_len s0 data inst addr pending len
          spec_st add_start i sec_cur t"
...
lemma flush_pending_outer_loop_preserves_inv_frame:
  fixes P :: "lifted_globals \<Rightarrow> bool"
  assumes inv0:
        "flush_pending_outer_loop_inv src_len s data inst addr pending len
          spec_st 0 0 sec s"
  ...
  subgoal by auto
  subgoal by simp
  subgoal using inv0 P0 by auto
  done

lemma flush_pending'_enc_sections_state_rel_branch_pre_frame:
  fixes P :: "lifted_globals \<Rightarrow> bool"
  assumes rel: "enc_sections_state_rel s data inst addr sec spec_st"
      and pending_eq: "enc_pending spec_st = heap_bytes_word s pending 0 len"
      and sec_ok: "sections_t_C.err_C sec = ENC_OK"
    ...
    show "flush_pending_outer_run_branch data data_cap inst inst_cap pending
          ...
          \<lbrace> \<lambda>r u.
               \<exists>sec'.
                 ...
                 P u \<rbrace>"
      by (rule run_pre[
          OF inv P_t i_le_len i_lt_j j_le_len run_end run_ge b_eq
             rel_cur eq_loop])
  qed
  show ?thesis
    unfolding flush_pending'_def flush_pending_outer_body_def
    ...
    proof -
      ...
      obtain loop_st where
          add_start_le_len: "add_start \<le> len"
        and rel_cur:
          "enc_sections_state_rel t data inst addr sec' loop_st"
        and eq_loop:
          "flush_pending_loop_spec src_len
            (heap_bytes_word s pending 0 len) (unat add_start) (unat len)
            loop_st =
           flush_pending_loop_spec src_len
            (heap_bytes_word s pending 0 len) 0 0 spec_st"
        using inv by (auto simp: flush_pending_outer_loop_inv_def)
      have tail:
        "flush_pending_outer_tail data data_cap inst inst_cap pending len
            add_start sec' \<bullet> t
         \<lbrace> \<lambda>Res sec'' u.
              ...
              P u \<rbrace>"
        by (rule tail_pre[
            OF inv P_t order_refl refl rel_cur eq_loop])
      have exit:
        "flush_pending_loop_spec src_len (heap_bytes_word s pending 0 len)
          (unat add_start) (unat len) loop_st =
         flush_pending_outer_tail_state src_len s pending len add_start
           loop_st"
        unfolding flush_pending_outer_tail_state_def
        by (rule flush_pending_loop_spec_exit_heap_emit_word[
          OF add_start_le_len]) simp
...
lemma try_emit_add_copy'_mode_le5_success_enc_cache_abs:
  fixes csz op pend_len copy_len :: "32 word"
    and m
  defines "csz \<equiv>
    (if (6 :: 32 word) < copy_len then (6 :: 32 word) else copy_len)"
...
lemma try_emit_add_copy'_mode_le5_success_enc_sections_cache_inv:
  ...
  shows "try_emit_add_copy' sec data data_cap inst inst_cap addr_buf addr_cap
           ...
           \<lbrace> \<lambda>r t.
              ...
              heap_typing t = heap_typing s \<rbrace>"
proof -
  have target_room_unfolded:
    "length target + unat pend_len +
      unat (if (6 :: 32 word) < copy_len then (6 :: 32 word) else copy_len)
      \<le> tgt_len"
    using target_room unfolding csz_def by simp
  have sections:
    "try_emit_add_copy' sec data data_cap inst inst_cap addr_buf addr_cap
        pending pend_len copy_addr here copy_len \<bullet> s
       \<lbrace> \<lambda>r t.
          ...
          heap_typing t = heap_typing s \<rbrace>"
    unfolding csz_def op_def
    by (rule try_emit_add_copy'_mode_le5_success_enc_sections_inv
      [OF inv abs cache_wf bm pend_ge pend_le copy_ge mode_le addr_size
          here_eq addr_ok target_room_unfolded sec_ok inst_byte_fits inst_byte_ptr
          inst_byte_dist inst_byte_data_disj inst_byte_addr_disj
          inst_byte_pending_disj data_fits data_valid pending_valid
          data_pending_disj data_inj data_prefix_disj data_no_overflow
          data_inst_disj data_addr_disj addr_varint_fits addr_varint_valid
          addr_varint_inj addr_varint_prefix_disj addr_varint_no_overflow
          addr_varint_data_disj addr_varint_inst_disj])
  have cache:
    "try_emit_add_copy' sec data data_cap inst inst_cap addr_buf addr_cap
        pending pend_len copy_addr here copy_len \<bullet> s
    ...
    by (rule try_emit_add_copy'_mode_le5_success_enc_cache_abs
      [OF abs cache_wf bm pend_ge pend_le copy_ge mode_le addr_size sec_ok
          inst_byte_fits inst_byte_ptr data_fits data_valid pending_valid
          addr_varint_fits addr_varint_valid])
  have combined:
    "try_emit_add_copy' sec data data_cap inst inst_cap addr_buf addr_cap
...
lemma try_emit_add_copy'_success_enc_sections_cache_inv_topdown:
  fixes csz op_gt op_le pend_len copy_len :: "32 word"
    and m
  defines "op_gt \<equiv>
    (235 + (mode_t_C.mode_C m - 6) * 4 + (pend_len - 1) :: 32 word)"
    and "csz \<equiv>
  ...
  assumes inv:
      ...
      and copy_ge: "(4 :: 32 word) \<le> copy_len"
      and mode_cases:
        "((5 :: 32 word) < mode_t_C.mode_C m \<and>
          mode_t_C.mode_C m \<le> (8 :: 32 word) \<and>
          copy_len = (4 :: 32 word)) \<or>
         mode_t_C.mode_C m \<le> (5 :: 32 word)"
      and addr_size: "varint_size' (mode_t_C.arg_C m) s = Some an"
      and here_eq:
        "unat here = length src_seg + length target + unat pend_len"
      and addr_ok:
        "unat copy_addr < length src_seg + length target + unat pend_len"
      and target_room_copy:
        "length target + unat pend_len + unat copy_len \<le> tgt_len"
      and target_room_csz:
        "length target + unat pend_len + unat csz \<le> tgt_len"
      and sec_ok: "sections_t_C.err_C sec = ENC_OK"
  ...
  shows "try_emit_add_copy' sec data data_cap inst inst_cap addr_buf addr_cap
            pending pend_len copy_addr here copy_len \<bullet> s
           \<lbrace> \<lambda>r t.
              (\<exists>f data_bytes' inst_bytes' addr_bytes' target' c_out'.
                r = Result f \<and>
                enc_sections_inv t data inst addr_buf (fused_t_C.s_C f)
                  src_seg tgt_len data_bytes' inst_bytes' addr_bytes'
                  target' c_out' \<and>
                enc_cache_abs t c_out' \<and>
                enc_cache_wf c_out') \<and>
              heap_typing t = heap_typing s \<rbrace>"
proof (cases "(5 :: 32 word) < mode_t_C.mode_C m")
  case True
  have mode_le: "mode_t_C.mode_C m \<le> (8 :: 32 word)"
    using mode_cases True
    by (auto simp: word_less_nat_alt word_le_nat_alt)
  ...
  have gt:
    "try_emit_add_copy' sec data data_cap inst inst_cap addr_buf addr_cap
       ...
       \<lbrace> \<lambda>r t.
          ...
          heap_typing t = heap_typing s \<rbrace>"
    unfolding op_gt_def
    by (rule try_emit_add_copy'_mode_gt5_success_enc_sections_cache_inv[
        OF inv abs cache_wf bm pend_ge pend_le copy_eq True mode_le
           here_eq addr_ok target_room_copy sec_ok inst_byte_fits
           inst_byte_ptr inst_byte_dist inst_byte_data_disj
           inst_byte_addr_disj inst_byte_pending_disj data_fits data_valid
           pending_valid data_pending_disj data_inj data_prefix_disj
           data_no_overflow data_inst_disj data_addr_disj addr_byte_fits
           ...
           addr_byte_inst_disj])
  show ?thesis
    apply (rule runs_to_weaken[OF gt])
    by blast
next
  case False
  have mode_le: "mode_t_C.mode_C m \<le> (5 :: 32 word)"
    using mode_cases False
    by (auto simp: word_less_nat_alt word_le_nat_alt)
  have target_room_unfolded:
    "length target + unat pend_len +
      unat (if (6 :: 32 word) < copy_len then (6 :: 32 word) else copy_len)
      \<le> tgt_len"
    using target_room_csz unfolding csz_def by simp
  have le:
    "try_emit_add_copy' sec data data_cap inst inst_cap addr_buf addr_cap
        pending pend_len copy_addr here copy_len \<bullet> s
       \<lbrace> \<lambda>r t.
          (\<exists>f.
            ...
            enc_cache_wf (cache_update c_out (unat copy_addr))) \<and>
          heap_typing t = heap_typing s \<rbrace>"
    unfolding csz_def op_le_def
    by (rule try_emit_add_copy'_mode_le5_success_enc_sections_cache_inv[
        OF inv abs cache_wf bm pend_ge pend_le copy_ge mode_le addr_size
           here_eq addr_ok target_room_unfolded sec_ok inst_byte_fits
           inst_byte_ptr inst_byte_dist inst_byte_data_disj
           inst_byte_addr_disj inst_byte_pending_disj data_fits data_valid
           pending_valid data_pending_disj data_inj data_prefix_disj
           data_no_overflow data_inst_disj data_addr_disj addr_varint_fits
           addr_varint_valid addr_varint_inj addr_varint_prefix_disj
           addr_varint_no_overflow addr_varint_data_disj
           addr_varint_inst_disj])
  show ?thesis
    apply (rule runs_to_weaken[OF le])
    by blast
qed
