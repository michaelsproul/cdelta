...
lemma which_lt2_slot_arith:
  fixes which :: "32 word"
  assumes "unat which < 2"
  shows "unat (which * 3) < 6"
    and "unat (which * 3 + 1) < 6"
    and "unat (which * 3 + (2 :: 32 word)) < 6"
    and "unat (1 - which) < unat ((2 :: 32 word) - which)"
proof -
  have cases: "which = 0 \<or> which = 1"
  proof -
  ...
  show "unat (which * 3) < 6"
    using cases by auto
  show "unat (which * 3 + 1) < 6"
    using cases by auto
  show "unat (which * 3 + (2 :: 32 word)) < 6"
    using cases by auto
  show "unat (1 - which) < unat ((2 :: 32 word) - which)"
    using cases by auto
qed

lemma ucast8_32_unat_lt_256:
  "unat (UCAST(8 \<rightarrow> 32) (b :: 8 word) :: 32 word) < 256"
proof -
  have "unat b < 256"
    using unat_lt2p[of b] by simp
  moreover have "unat (UCAST(8 \<rightarrow> 32) b :: 32 word) = unat b"
    using calculation by (simp add: unat_ucast)
  ultimately show ?thesis by simp
qed

lemma word8_unat_le3_cases:
  assumes "unat (b :: 8 word) \<le> 3"
  shows "b = 0 \<or> b = 1 \<or> b = 2 \<or> b = 3"
proof -
  have cases: "unat b = 0 \<or> unat b = 1 \<or> unat b = 2 \<or> unat b = 3"
    using assms by arith
  show ?thesis
  proof (rule disjE[OF cases])
    assume h: "unat b = 0"
    have "unat b = unat (0 :: 8 word)"
      using h by simp
    hence "b = (0 :: 8 word)"
      by (rule word_unat.Rep_inject[THEN iffD1])
    thus ?thesis by simp
  next
    assume rest: "unat b = 1 \<or> unat b = 2 \<or> unat b = 3"
    show ?thesis
    proof (rule disjE[OF rest])
      assume h: "unat b = 1"
      hence "unat b = unat (1 :: 8 word)" by simp
      hence "b = (1 :: 8 word)"
        by (rule word_unat.Rep_inject[THEN iffD1])
      thus ?thesis by simp
    next
      assume rest': "unat b = 2 \<or> unat b = 3"
      show ?thesis
      proof (rule disjE[OF rest'])
        assume h: "unat b = 2"
        hence "unat b = unat (2 :: 8 word)" by simp
        hence "b = (2 :: 8 word)"
          by (rule word_unat.Rep_inject[THEN iffD1])
        thus ?thesis by simp
      next
        assume h: "unat b = 3"
        hence "unat b = unat (3 :: 8 word)" by simp
        hence "b = (3 :: 8 word)"
          by (rule word_unat.Rep_inject[THEN iffD1])
        thus ?thesis by simp
      qed
...
lemma ucast8_32_eq_of_nat:
  fixes b :: "8 word"
  assumes eq: "UCAST(8 \<rightarrow> 32) b = (of_nat n :: 32 word)"
      and n_lt: "n < 256"
  shows "b = of_nat n"
proof -
  have unat_b: "unat b = n"
  proof -
    have "unat (UCAST(8 \<rightarrow> 32) b :: 32 word) =
          unat (of_nat n :: 32 word)"
      using eq by simp
    thus ?thesis
      using n_lt by (simp add: unat_ucast_upcast is_up unat_of_nat_eq)
  qed
  have "unat (of_nat n :: 8 word) = n"
    using n_lt by (simp add: unat_of_nat_eq)
  with unat_b show ?thesis
...
lemma decode_address'_spec_ex:
  assumes buf_ok: "buf_valid s patch (unat addr_end)"
      and pos_ok: "pos \<le> addr_end"
      and cache_ok: "\<exists>c. cache_abs s c np_in \<and> cache_wf c"
      and mode_ok: "unat mode < 9"
      and here_ok: "unat here < 2 ^ 32"
  shows "decode_address' patch addr_end pos here mode np_in \<bullet> s
         \<lbrace> \<lambda>r s'.
            \<exists>c ar. cache_abs s c np_in \<and> cache_wf c \<and>
              ...
              (let bytes = heap_bytes s patch (unat addr_end);
               ...
               in (case decoded of
                   ...
                   | None \<Rightarrow> ar_t_C.err_C ar \<noteq> 0)) \<rbrace>"
proof -
  obtain c where cache_abs_c: "cache_abs s c np_in"
    and cache_wf_c: "cache_wf c"
    using cache_ok by blast
  show ?thesis
    apply (rule runs_to_weaken
      [OF decode_address'_spec
        [OF buf_ok pos_ok cache_abs_c cache_wf_c mode_ok here_ok]])
    using cache_abs_c cache_wf_c
    by blast
qed
...
definition code_tbl_tags_valid :: "lifted_globals \<Rightarrow> bool" where
  "code_tbl_tags_valid s =
     (\<forall>op < 256.
        unat (code_tbl_'' s .[op] .[0]) \<le> 3 \<and>
        unat (code_tbl_'' s .[op] .[3]) \<le> 3)"
...
lemma code_tbl_tags_valid_heap_w8_update[simp]:
  "code_tbl_tags_valid (heap_w8_update f s) = code_tbl_tags_valid s"
...
lemma code_tbl_tags_valid_heap_w32_update[simp]:
  "code_tbl_tags_valid (heap_w32_update f s) = code_tbl_tags_valid s"
  by (simp add: code_tbl_tags_valid_def)
...
lemma code_tbl_tags_valid_near_arr_update[simp]:
  "code_tbl_tags_valid (near_arr_''_update f s) = code_tbl_tags_valid s"
  by (simp add: code_tbl_tags_valid_def)
...
lemma code_tbl_tags_valid_same_arr_update[simp]:
  "code_tbl_tags_valid (same_arr_''_update f s) = code_tbl_tags_valid s"
  by (simp add: code_tbl_tags_valid_def)
...
lemma code_tbl_tags_valid_code_tbl_built_update[simp]:
  "code_tbl_tags_valid (code_tbl_built_''_update f s) = code_tbl_tags_valid s"
  by (simp add: code_tbl_tags_valid_def)
...
lemma add_loop_correct_c:
  fixes sz :: "32 word" and data_cursor :: "32 word"
    and tgt_pos :: "32 word" and patch :: "8 word ptr"
    and src :: "8 word ptr" and out :: "8 word ptr"
  assumes data_valid: "\<forall>j < unat sz.
           ptr_valid (heap_typing s) (patch +\<^sub>p uint (data_cursor + of_nat j))"
      and out_valid: "\<forall>j < unat sz.
           ptr_valid (heap_typing s) (out +\<^sub>p uint (tgt_pos + of_nat j))"
      and disj: "\<forall>i < unat sz. \<forall>j < patch_n.
           out +\<^sub>p uint (tgt_pos + of_nat i) \<noteq> patch +\<^sub>p int j"
      and src_disj: "\<forall>i < src_n. \<forall>j < unat sz.
           src +\<^sub>p int i \<noteq> out +\<^sub>p uint (tgt_pos + of_nat j)"
      and out_prefix_disj: "\<forall>i < unat tgt_pos. \<forall>j < unat sz.
           out +\<^sub>p int i \<noteq> out +\<^sub>p uint (tgt_pos + of_nat j)"
      and out_inj: "\<forall>i < unat sz. \<forall>j < unat sz.
           i \<noteq> j \<longrightarrow> out +\<^sub>p uint (tgt_pos + of_nat i) \<noteq> out +\<^sub>p uint (tgt_pos + of_nat j)"
      and no_overflow_data: "unat data_cursor + unat sz < 2 ^ 32"
      and no_overflow_tgt: "unat tgt_pos + unat sz < 2 ^ 32"
  ...
  shows "(whileLoop (\<lambda>(j :: 32 word) st. j < sz)
           (\<lambda>j. do {
              ...
              modify (heap_w8_update
                (\<lambda>h. h(out +\<^sub>p uint (tgt_pos + j) := h (patch +\<^sub>p uint (data_cursor + j)))));
              return (j + 1)
           }) (0 :: 32 word) :: (32 word, lifted_globals) res_monad) \<bullet> s
         \<lbrace> \<lambda>r t. r = Result sz \<and>
            (\<forall>j < patch_n. heap_w8 t (patch +\<^sub>p int j) = heap_w8 s (patch +\<^sub>p int j)) \<and>
  ...
  apply (rule runs_to_whileLoop_res'[
    where R = "measure (\<lambda>((j :: 32 word), _). unat sz - unat j)"
      and I = "\<lambda>j st. unat j \<le> unat sz
             ...
             \<and> (\<forall>k < unat tgt_pos. heap_w8 st (out +\<^sub>p int k) = heap_w8 s (out +\<^sub>p int k))
             \<and> (\<forall>k < unat j. heap_w8 st (out +\<^sub>p uint (tgt_pos + of_nat k)) =
                  heap_w8 s (patch +\<^sub>p uint (data_cursor + of_nat k)))
             \<and> heap_typing st = heap_typing s
             \<and> near_arr_'' st = near_arr_'' s
             \<and> same_arr_'' st = same_arr_'' s
             \<and> code_tbl_'' st = code_tbl_'' s"])
     subgoal by simp
    subgoal by (simp add: word_less_nat_alt)
   subgoal for j st
     by (clarsimp simp: word_less_nat_alt)
  subgoal for j st
    ...
    subgoal by auto
    subgoal for k
      using src_disj[rule_format, of k "unat j"]
      by (auto simp: word_less_nat_alt word_unat.Rep_inverse)
    subgoal by auto
    subgoal for k
      using out_prefix_disj[rule_format, of k "unat j"]
      by (auto simp: word_less_nat_alt word_unat.Rep_inverse)
    subgoal by auto
    subgoal for k
      apply (subgoal_tac "k = unat j")
...
lemma run_loop_correct:
  fixes sz :: "32 word" and tgt_pos :: "32 word"
    ...
    and src :: "8 word ptr"
  assumes out_valid: "\<forall>j < unat sz.
           ptr_valid (heap_typing s) (out +\<^sub>p uint (tgt_pos + of_nat j))"
      and disj: "\<forall>i < unat sz. \<forall>j < patch_n.
           out +\<^sub>p uint (tgt_pos + of_nat i) \<noteq> patch +\<^sub>p int j"
      and src_disj: "\<forall>i < src_n. \<forall>j < unat sz.
    ...
    \<comment> \<open>3. Patch preservation (equal ptr - impossible by disj)\<close>
      subgoal for k
	    ...
	    \<comment> \<open>4. Patch preservation (not-equal - trivial from IH)\<close>
	     subgoal by auto
	    \<comment> \<open>5. Source preservation\<close>
	    subgoal for k
	      using src_disj[rule_format, of k "unat j"]
	      by (auto simp: word_less_nat_alt word_unat.Rep_inverse)
	     subgoal by auto
	    \<comment> \<open>6. Output prefix preservation\<close>
	    subgoal for k
	      using out_prefix_disj[rule_format, of k "unat j"]
	      by (auto simp: word_less_nat_alt word_unat.Rep_inverse)
	     subgoal by auto
	    \<comment> \<open>7. Output correct (not-equal ptr): k < unat j from IH\<close>
	    subgoal for k
	      apply (subgoal_tac "k < unat j")
	       apply auto[1]
	      apply (cases "k = unat j")
...
lemma unat_plus_one_under_tgt_bound:
  fixes j sz tgt_pos :: "32 word"
  assumes "unat j < unat sz"
      and "unat tgt_pos + unat sz < 2 ^ 32"
  shows "unat (j + 1 :: 32 word) = Suc (unat j)"
proof -
  have "unat j + 1 < 2 ^ 32"
  ...
  fixes sz :: "32 word" and tgt_pos :: "32 word"
    ...
    and src_seg :: "8 word list" and tgt_pre :: "8 word list"
	  ...
	  apply (rule runs_to_whileLoop_exn'[
	    where R = "measure (\<lambda>((j :: 32 word), _). unat sz - unat j)"
	      and I = "\<lambda>r st.
	        ((\<forall>j. r = Result j \<longrightarrow>
	            unat j \<le> unat sz
	            \<and> (\<forall>k < patch_n. heap_w8 st (patch +\<^sub>p int k) = heap_w8 s (patch +\<^sub>p int k))
	            \<and> (\<forall>k < src_n. heap_w8 st (src +\<^sub>p int k) = heap_w8 s (src +\<^sub>p int k))
	            \<and> (\<forall>k < unat j.
	            ...
	            \<and> same_arr_'' st = same_arr_'' s
	            \<and> code_tbl_'' st = code_tbl_'' s)
	         \<and> (\<forall>e. r = Exn e \<longrightarrow> False))"])
	  subgoal for j st
	    apply (clarsimp simp: Let_def)
	    apply (cases "unat (addr + j) < unat src_seg_len")
    subgoal \<comment> \<open>Source branch: read from src[src_seg_off + addr + j]\<close>
      ...
      \<comment> \<open>2. out guard\<close>
            subgoal using out_write_valid[rule_format, of "unat j"]
		      ...
		      \<comment> \<open>3. bound\<close>
		           subgoal
		             apply (subgoal_tac "unat j + 1 < 2 ^ 32")
		              apply (simp add: unat_word_ariths(1))
		             using no_overflow_tgt by linarith
         ...
         apply (subgoal_tac "heap_w8 s (src +\<^sub>p uint (src_seg_off + (addr + j))) =
          ...
          apply (subgoal_tac "unat (j + 1) = Suc (unat j)")
           ...
           using tgt_pre_def apply (simp add: heap_bytes_def)
	          apply (rule unat_plus_one_under_tgt_bound)
	           apply assumption
	          using no_overflow_tgt apply assumption
      ...
      subgoal for k
        ...
        apply (cases "k = unat j")
         apply (simp add: word_unat.Rep_inverse)
		        apply (subgoal_tac "unat (j + 1 :: 32 word) = Suc (unat j)")
	         apply linarith
	        apply (rule unat_plus_one_under_tgt_bound)
	         apply assumption
	        using no_overflow_tgt by assumption
      ...
      subgoal
	    ...
	    subgoal \<comment> \<open>Overlapping copy branch: read from out[addr + j - src_seg_len]\<close>
	      apply simp
	      apply (subgoal_tac "\<not> unat (tgt_pos + j) \<le> unat (addr + j - src_seg_len)")
	       prefer 2
	       apply (subgoal_tac "unat (addr + j) = unat addr + unat j")
	        prefer 2
	        apply (rule unat_add_no_overflow)
	       ...
	       apply (subgoal_tac "unat (tgt_pos + j) = unat tgt_pos + unat j")
	        prefer 2
	        apply (rule unat_add_no_overflow)
	        using no_overflow_tgt apply linarith
	       apply (subgoal_tac "src_seg_len \<le> addr + j")
	        prefer 2
	        apply (simp add: word_le_nat_alt)
	       apply (subgoal_tac "unat (addr + j - src_seg_len) =
	                            unat addr + unat j - unat src_seg_len")
	        prefer 2
	        ...
	        apply (subgoal_tac "addr + j - src_seg_len =
	         ...
	         apply (rule copy_loop_overlap_ptr_eq)
	            apply assumption
		          ...
		          apply simp
			         apply (subgoal_tac
			           "unat addr + unat j - unat src_seg_len - unat tgt_pos < unat j")
		          using no_overflow_tgt apply linarith
		         using addr_bound apply linarith
      ...
      \<comment> \<open>12. termination\<close>
		  ...
		  subgoal for j st by clarsimp
	  subgoal for e st by simp
	  subgoal by simp
	  subgoal by simp
	  done
  ...
  "decode_loop_inv s0 patch patch_n src src_n out src_seg_off src_seg_len tgt_len
   ...
   (\<exists>dst :: dec_state. \<exists>c :: cache.
      ...
      (\<forall>i < tgt_len. \<forall>j < tgt_len. i \<noteq> j \<longrightarrow> out +\<^sub>p int i \<noteq> out +\<^sub>p int j) \<and>
		      ...
		      unat src_seg_len + tgt_len < 2 ^ 32 \<and>
		      \<comment> \<open>Source segment definition\<close>
		      src_seg = heap_bytes s0 (src +\<^sub>p uint src_seg_off) (unat src_seg_len) \<and>
		      code_tbl_tags_valid t)"
...
lemma decode_loop_inv_core_imp_inv:
  assumes "decode_loop_inv_core s0 patch patch_n src src_n out
             src_seg_off src_seg_len tgt_len
             data_end inst_end addr_end src_seg
             data_cursor inst_cursor addr_cursor tgt_pos np dst c t"
  shows "decode_loop_inv s0 patch patch_n src src_n out
           src_seg_off src_seg_len tgt_len
           data_end inst_end addr_end src_seg
           data_cursor inst_cursor addr_cursor tgt_pos np t"
  using assms unfolding decode_loop_inv_eq_core by blast

lemma decode_loop_inv_core_inst_rem_current:
  assumes core:
    "decode_loop_inv_core s0 patch patch_n src src_n out
       src_seg_off src_seg_len tgt_len
       data_end inst_end addr_end src_seg
       data_cursor inst_cursor addr_cursor tgt_pos np dst c t"
  shows "ds_inst_rem dst =
         drop (unat inst_cursor) (heap_bytes t patch (unat inst_end))"
proof -
  have inst_rem:
    "ds_inst_rem dst =
  ...
  have inst_end_bound: "unat inst_end \<le> patch_n"
    using core unfolding decode_loop_inv_core_def by simp
  have patch_preserved:
    "\<forall>i < patch_n. heap_w8 t (patch +\<^sub>p int i) = heap_w8 s0 (patch +\<^sub>p int i)"
    using core unfolding decode_loop_inv_core_def by simp
  have bytes_eq:
    "heap_bytes t patch (unat inst_end) =
     heap_bytes s0 patch (unat inst_end)"
    by (rule heap_bytes_eqI)
       (use patch_preserved inst_end_bound in auto)
  have prefix:
    "take (unat inst_end) (heap_bytes s0 patch patch_n) =
     heap_bytes s0 patch (unat inst_end)"
    by (rule heap_bytes_prefix[OF inst_end_bound])
	  show ?thesis
	    using inst_rem prefix bytes_eq by simp
qed

lemma decode_loop_inv_core_addr_rem_current:
  assumes core:
    "decode_loop_inv_core s0 patch patch_n src src_n out
       src_seg_off src_seg_len tgt_len
       data_end inst_end addr_end src_seg
       data_cursor inst_cursor addr_cursor tgt_pos np dst c t"
  shows "ds_addr_rem dst =
         drop (unat addr_cursor) (heap_bytes t patch (unat addr_end))"
proof -
  have addr_rem:
    "ds_addr_rem dst =
       drop (unat addr_cursor) (take (unat addr_end) (heap_bytes s0 patch patch_n))"
    using core unfolding decode_loop_inv_core_def by simp
  have addr_end_bound: "unat addr_end \<le> patch_n"
    using core unfolding decode_loop_inv_core_def by simp
  have patch_preserved:
    "\<forall>i < patch_n. heap_w8 t (patch +\<^sub>p int i) = heap_w8 s0 (patch +\<^sub>p int i)"
    using core unfolding decode_loop_inv_core_def by simp
  have bytes_eq:
    "heap_bytes t patch (unat addr_end) =
     heap_bytes s0 patch (unat addr_end)"
    by (rule heap_bytes_eqI)
       (use patch_preserved addr_end_bound in auto)
  have prefix:
    "take (unat addr_end) (heap_bytes s0 patch patch_n) =
     heap_bytes s0 patch (unat addr_end)"
    by (rule heap_bytes_prefix[OF addr_end_bound])
  show ?thesis
    using addr_rem prefix bytes_eq by simp
qed

lemma decode_loop_inv_core_unique:
  assumes core1:
    "decode_loop_inv_core s0 patch patch_n src src_n out
       src_seg_off src_seg_len tgt_len
       data_end inst_end addr_end src_seg
       data_cursor inst_cursor addr_cursor tgt_pos np dst1 c1 t"
  assumes core2:
    "decode_loop_inv_core s0 patch patch_n src src_n out
       src_seg_off src_seg_len tgt_len
       data_end inst_end addr_end src_seg
       data_cursor inst_cursor addr_cursor tgt_pos np dst2 c2 t"
  shows "dst1 = dst2 \<and> c1 = c2"
proof -
  have fields:
    "ds_inst_rem dst1 = ds_inst_rem dst2"
    "ds_data_rem dst1 = ds_data_rem dst2"
    "ds_addr_rem dst1 = ds_addr_rem dst2"
    "ds_tgt dst1 = ds_tgt dst2"
    using core1 core2 unfolding decode_loop_inv_core_def by auto
  have c_eq: "c1 = c2"
  proof -
    have "cache_abs t c1 np" "cache_wf c1"
      using core1 unfolding decode_loop_inv_core_def by auto
    moreover have "cache_abs t c2 np" "cache_wf c2"
      using core2 unfolding decode_loop_inv_core_def by auto
    ultimately show ?thesis
      by (rule cache_abs_unique)
  qed
  have "ds_cache dst1 = ds_cache dst2"
    using core1 core2 c_eq unfolding decode_loop_inv_core_def by auto
  with fields have "dst1 = dst2"
    by (cases dst1; cases dst2; simp)
  with c_eq show ?thesis by simp
qed

lemma decode_loop_inv_core_advance_inst:
  assumes core:
    "decode_loop_inv_core s0 patch patch_n src src_n out
       src_seg_off src_seg_len tgt_len
       data_end inst_end addr_end src_seg
  ...
  shows "decode_loop_inv_core s0 patch patch_n src src_n out
           src_seg_off src_seg_len tgt_len
           data_end inst_end addr_end src_seg
           data_cursor (inst_cursor + 1) addr_cursor tgt_pos np
...
proof -
  ...
  proof -
    have "Suc (unat inst_cursor) \<le> unat inst_end"
      using ic_lt by simp
  ...
  proof -
    have "drop (Suc (unat inst_cursor))
      ...
      by (simp add: drop_Suc tl_drop)
    thus ?thesis
      using core unat_ic1 unfolding decode_loop_inv_core_def by simp
  qed
  show ?thesis
    using core new_inst ic1_le
    unfolding decode_loop_inv_core_def
    by simp
qed

definition decode_one_prefix ::
  "byte list \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> dec_state \<Rightarrow> nat \<Rightarrow> dec_state \<Rightarrow> bool" where
  "decode_one_prefix src_seg src_seg_len tgt_len dst which st =
   (case pop_byte (ds_inst_rem dst) of
      None \<Rightarrow> False
    | Some (op, irest) \<Rightarrow>
        (let h1 = fst (default_entry (unat op));
             h2 = snd (default_entry (unat op));
             st1 = dst\<lparr>ds_inst_rem := irest\<rparr>
         in if which = 0 then st = st1
              ...
              (case resolve_size h1 (ds_inst_rem st1) of
               ...
               | Some (sz1, irest1) \<Rightarrow>
                   exec_half h1 sz1 src_seg src_seg_len tgt_len
                     (st1\<lparr>ds_inst_rem := irest1\<rparr>) = Inl st)
            else if which = 2 then
              decode_one src_seg src_seg_len tgt_len dst = Inl st
            else False))"

definition decode_inner_inv_core ::
  "lifted_globals \<Rightarrow> 8 word ptr \<Rightarrow> nat \<Rightarrow> 8 word ptr \<Rightarrow> nat \<Rightarrow>
   8 word ptr \<Rightarrow> 32 word \<Rightarrow> 32 word \<Rightarrow> nat \<Rightarrow>
   32 word \<Rightarrow> 32 word \<Rightarrow> 32 word \<Rightarrow>
   byte list \<Rightarrow> dec_state \<Rightarrow>
  ...
  "decode_inner_inv_core s0 patch patch_n src src_n out
   ...
   (which \<le> (2 :: 32 word) \<and>
    (\<exists>dst c.
      decode_loop_inv_core s0 patch patch_n src src_n out
        src_seg_off src_seg_len tgt_len
        data_end inst_end addr_end src_seg
        data_cursor inst_cursor addr_cursor tgt_pos np dst c t \<and>
      decode_one_prefix src_seg (unat src_seg_len) tgt_len dst0 (unat which) dst))"

lemma decode_one_prefix_0I:
  assumes "pop_byte (ds_inst_rem dst) = Some (op, irest)"
      and "st = dst\<lparr>ds_inst_rem := irest\<rparr>"
  shows "decode_one_prefix src_seg src_seg_len tgt_len dst 0 st"
  using assms by (simp add: decode_one_prefix_def Let_def)

lemma decode_one_prefix_2D:
  assumes "decode_one_prefix src_seg src_seg_len tgt_len dst 2 st"
  shows "decode_one src_seg src_seg_len tgt_len dst = Inl st"
  using assms by (auto simp: decode_one_prefix_def Let_def split: option.splits prod.splits)

lemma decode_one_prefix_noop_advance:
  fixes which :: "32 word"
  assumes pop: "pop_byte (ds_inst_rem dst0) = Some (op, irest)"
      and prefix: "decode_one_prefix src_seg src_seg_len tgt_len dst0 (unat which) st"
      and which_lt: "unat which < 2"
      and noop_first: "which = 0 \<Longrightarrow> ity (fst (default_entry (unat op))) = NOOP"
      and noop_second: "which = 1 \<Longrightarrow> ity (snd (default_entry (unat op))) = NOOP"
  shows "decode_one_prefix src_seg src_seg_len tgt_len dst0 (unat (which + 1)) st"
proof -
  have cases: "which = 0 \<or> which = 1"
  proof -
    have "unat which = 0 \<or> unat which = 1"
  ...
  proof
    ...
    let ?h1 = "fst (default_entry (unat op))"
    let ?st1 = "dst0\<lparr>ds_inst_rem := irest\<rparr>"
    have which_simp: "unat (which + 1) = 1"
      using which0 by simp
    have st_eq: "st = ?st1"
      using prefix pop which0
      by (simp add: decode_one_prefix_def Let_def)
    have "exec_half ?h1 (isz ?h1) src_seg src_seg_len tgt_len ?st1 = Inl ?st1"
      using noop_first[OF which0] by (simp add: exec_half_def)
    with pop st_eq which_simp noop_first[OF which0] show ?thesis
      by (simp add: decode_one_prefix_def resolve_size_def Let_def)
  next
    assume which1: "which = 1"
    let ?h1 = "fst (default_entry (unat op))"
    let ?h2 = "snd (default_entry (unat op))"
    let ?st1 = "dst0\<lparr>ds_inst_rem := irest\<rparr>"
    have which_simp: "unat (which + 1) = 2"
      using which1 by simp
    obtain sz1 irest1 where
      resolve1: "resolve_size ?h1 (ds_inst_rem ?st1) = Some (sz1, irest1)"
      and exec1:
        "exec_half ?h1 sz1 src_seg src_seg_len tgt_len
          (?st1\<lparr>ds_inst_rem := irest1\<rparr>) = Inl st"
      using prefix pop which1
      by (auto simp: decode_one_prefix_def Let_def split: option.splits)
    have exec2:
      "exec_half ?h2 (isz ?h2) src_seg src_seg_len tgt_len st = Inl st"
      using noop_second[OF which1] by (simp add: exec_half_def)
    have resolve2:
      "resolve_size ?h2 (ds_inst_rem st) = Some (isz ?h2, ds_inst_rem st)"
      using noop_second[OF which1] by (simp add: resolve_size_def)
    have "decode_one src_seg src_seg_len tgt_len dst0 = Inl st"
      using pop resolve1 exec1 resolve2 exec2
      by (auto simp add: decode_one_def Let_def split: prod.splits)
    with pop which_simp show ?thesis
      by (simp add: decode_one_prefix_def Let_def)
  qed
qed

lemma decode_one_prefix_resolve_size_some:
  fixes which :: "32 word"
  assumes pop: "pop_byte (ds_inst_rem dst0) = Some (op, irest)"
      and prefix: "decode_one_prefix src_seg src_seg_len tgt_len dst0 (unat which) st"
      and whole: "decode_one src_seg src_seg_len tgt_len dst0 = Inl dst_final"
      and which_lt: "unat which < 2"
  shows "\<exists>sz rest.
           resolve_size
             (if which = 0
              then fst (default_entry (unat op))
              else snd (default_entry (unat op)))
             (ds_inst_rem st) = Some (sz, rest)"
proof -
  ...
  proof (rule disjE[OF which_cases])
    ...
    have st_eq: "st = ?st1"
      ...
      by (simp add: decode_one_prefix_def Let_def)
    obtain sz1 irest1 where
      "resolve_size ?h1 (ds_inst_rem ?st1) = Some (sz1, irest1)"
      using whole pop
      by (auto simp: decode_one_def Let_def
          split: option.splits prod.splits sum.splits)
    thus ?thesis using st_eq which0 by simp
  next
    assume which1: "which = 1"
    let ?h1 = "fst (default_entry (unat op))"
    let ?h2 = "snd (default_entry (unat op))"
    ...
    obtain sz2 irest2 where
      "resolve_size ?h2 (ds_inst_rem st) = Some (sz2, irest2)"
      using whole pop resolve1 exec1
      by (auto simp: decode_one_def Let_def
          split: option.splits prod.splits sum.splits)
    thus ?thesis using which1 by simp
  qed
qed

lemma decode_one_prefix_exec_half_some:
  fixes which :: "32 word"
  assumes pop: "pop_byte (ds_inst_rem dst0) = Some (op, irest)"
      and prefix: "decode_one_prefix src_seg src_seg_len tgt_len dst0 (unat which) st"
      and whole: "decode_one src_seg src_seg_len tgt_len dst0 = Inl dst_final"
  ...
  shows "\<exists>st'.
          exec_half
            ...
            sz src_seg src_seg_len tgt_len
            (st\<lparr>ds_inst_rem := rest\<rparr>) = Inl st'"
proof -
  have which_cases: "which = 0 \<or> which = 1"
  proof -
    have "unat which = 0 \<or> unat which = 1"
  ...
  proof (rule disjE[OF which_cases])
    ...
    have st_eq: "st = ?st1"
      ...
      by (simp add: decode_one_prefix_def Let_def)
    show ?thesis
      using whole pop resolve_cur st_eq which0
      by (auto simp: decode_one_def Let_def
          split: option.splits prod.splits sum.splits)
  next
    assume which1: "which = 1"
    ...
    obtain sz1 irest1 where
      ...
      by (auto simp: decode_one_prefix_def Let_def split: option.splits)
    show ?thesis
      using whole pop resolve1 exec1 resolve_cur which1
      by (auto simp: decode_one_def Let_def
          split: option.splits prod.splits sum.splits)
  qed
qed

lemma decode_one_prefix_exec_half_advance:
  fixes which :: "32 word"
  assumes pop: "pop_byte (ds_inst_rem dst0) = Some (op, irest)"
      and prefix: "decode_one_prefix src_seg src_seg_len tgt_len dst0 (unat which) st"
      and which_lt: "unat which < 2"
...
proof -
  ...
  proof (rule disjE[OF which_cases])
    ...
    have which_simp: "unat (which + 1) = 1"
      using which0 by simp
    show ?thesis
      using pop resolve_cur exec_cur st_eq which0 which_simp
      by (simp add: decode_one_prefix_def Let_def)
  next
    assume which1: "which = 1"
    let ?h1 = "fst (default_entry (unat op))"
    ...
    obtain sz1 irest1 where
      ...
      using prefix pop which1
      by (auto simp: decode_one_prefix_def Let_def split: option.splits)
    have whole: "decode_one src_seg src_seg_len tgt_len dst0 = Inl st'"
      using pop resolve1 exec1 resolve_cur exec_cur which1
      by (auto simp: decode_one_def Let_def split: prod.splits)
    have which_simp: "unat (which + 1) = 2"
      using which1 by simp
    show ?thesis
      using whole pop which_simp
      by (simp add: decode_one_prefix_def Let_def)
  qed
qed

lemma exec_half_nonnoop_tgt_fit:
  assumes nonnoop: "ity h \<noteq> NOOP"
      and exec: "exec_half h sz src_seg src_seg_len tgt_len st = Inl st'"
  shows "length (ds_tgt st) + sz \<le> tgt_len"
  using assms
  by (cases "ity h")
     (auto simp: exec_half_def split: if_splits option.splits prod.splits)

lemma decode_loop_inv_plus_decode_oneD:
  assumes inv: "decode_loop_inv_plus s0 patch patch_n src src_n out
                  src_seg_off src_seg_len tgt_len
                  data_end inst_end addr_end src_seg tgt
                  data_cursor inst_cursor addr_cursor tgt_pos np t"
      and progressing: "inst_cursor < inst_end"
  obtains dst c dst' dst_final k where
    "decode_loop_inv_core s0 patch patch_n src src_n out
       src_seg_off src_seg_len tgt_len
       data_end inst_end addr_end src_seg
       data_cursor inst_cursor addr_cursor tgt_pos np dst c t"
    "decode_one src_seg (unat src_seg_len) tgt_len dst = Inl dst'"
    "decode_loop k src_seg (unat src_seg_len) tgt_len dst' = Inl dst_final"
    "length (ds_tgt dst_final) = tgt_len"
    "ds_tgt dst_final = tgt"
    "ds_data_rem dst_final = []"
    "ds_addr_rem dst_final = []"
proof -
  obtain dst c dst_final k_prev where
    core: "decode_loop_inv_core s0 patch patch_n src src_n out
             src_seg_off src_seg_len tgt_len
             data_end inst_end addr_end src_seg
             data_cursor inst_cursor addr_cursor tgt_pos np dst c t"
    and decloop:
      "decode_loop k_prev src_seg (unat src_seg_len) tgt_len dst = Inl dst_final"
    and final:
      "length (ds_tgt dst_final) = tgt_len"
      "ds_tgt dst_final = tgt"
      "ds_data_rem dst_final = []"
      "ds_addr_rem dst_final = []"
    using decode_loop_inv_plus_coreD[OF inv] by blast
  ...
  proof (rule ccontr)
    ...
    with decloop inst_nonempty show False by simp
  qed
  then obtain k where k_eq: "k_prev = Suc k"
    by (cases k_prev) auto
  obtain dst' where step:
    "decode_one src_seg (unat src_seg_len) tgt_len dst = Inl dst'"
    and rec:
    "decode_loop k src_seg (unat src_seg_len) tgt_len dst' = Inl dst_final"
  proof (cases "decode_one src_seg (unat src_seg_len) tgt_len dst")
    case (Inl a)
    with decloop k_eq inst_nonempty show ?thesis
      by (intro that[of a]) simp_all
  next
    case (Inr e)
    with decloop k_eq inst_nonempty show ?thesis by simp
  qed
  show ?thesis
    by (rule that[OF core step rec final])
qed
...
lemma outer_whileLoop_correct_abstract:
  ...
  assumes inv_entry:
    "decode_loop_inv_plus base patch patch_n src src_n out
       src_seg_off src_seg_len tgt_len
       data_end inst_end addr_end src_seg tgt
...
lemma decode_loop_inv_init:
  assumes heap_eq: "\<forall>i < patch_n. heap_w8 t (patch +\<^sub>p int i) = heap_w8 s0 (patch +\<^sub>p int i)"
      ...
      and cursor_bounds: "data_cursor \<le> data_end" "inst_cursor \<le> inst_end"
	      ...
	      and code_tbl_tags: "code_tbl_tags_valid t"
      ...
      and out_inj: "\<forall>i < tgt_len. \<forall>j < tgt_len. i \<noteq> j \<longrightarrow> out +\<^sub>p int i \<noteq> out +\<^sub>p int j"
	      ...
	      and src_tgt_len_fits: "unat src_seg_len + tgt_len < 2 ^ 32"
...
proof -
  ...
  show ?thesis
    ...
    using heap_eq src_eq patch_valid src_valid out_valid cache_init_abs
		          cursor_bounds end_bounds code_tbl_ok code_tbl_tags typing_eq out_disj_patch out_disj_src
		          out_inj tgt_len_fits src_seg_bounds src_tgt_len_fits src_seg_eq wf tgt0
		    by simp
qed
    ...
    "decode_loop_inv_core s0 patch patch_n src src_n out
       ...
       ?dst0 cache_init t"
		    by (rule decode_loop_inv_init_core[OF patch_eq src_eq patch_valid src_valid
			          out_valid cache_init_abs cursor_bounds end_bounds code_tbl_ok code_tbl_tags typing_eq
			          out_disj_patch out_disj_src out_inj tgt_len_fits src_seg_bounds
...
proof -
  ...
  have src_tgt_fit_inv: "unat src_seg_len + tgt_len < 2 ^ 32"
    using inv unfolding decode_loop_inv_def by auto
  ...
  show ?thesis
    ...
    \<comment> \<open>src_seg_off + src_seg_len \<le> src_n\<close>
	    ...
	    \<comment> \<open>src_seg_len + tgt_len < 2^32\<close>
	    using src_tgt_fit_inv apply simp
...
lemma code_tbl_first_tag_zero_noop:
  assumes "code_tbl_matches t"
      and "unat (op :: 8 word) < 256"
      and "UCAST(8 \<rightarrow> 32) (code_tbl_'' t .[unat op] .[0]) = (0 :: 32 word)"
  shows "ity (fst (default_entry (unat op))) = NOOP"
proof -
  have tag0: "code_tbl_'' t .[unat op] .[0] = (0 :: 8 word)"
  proof -
    have "unat (UCAST(8 \<rightarrow> 32) (code_tbl_'' t .[unat op] .[0]) :: 32 word) = 0"
      using assms(3) by simp
    hence "unat (code_tbl_'' t .[unat op] .[0]) = 0"
      by (simp add: unat_ucast_upcast is_up)
    thus ?thesis by (simp add: unat_eq_0)
  qed
  have h_eq: "byte_to_hi (code_tbl_'' t .[unat op] .[0])
                   (code_tbl_'' t .[unat op] .[1])
                   (code_tbl_'' t .[unat op] .[2])
      = fst (default_entry (unat op))"
    by (rule code_tbl_matches_first_half[OF assms(1,2)])
  show ?thesis
    using h_eq[symmetric] tag0 by (simp add: byte_to_hi_def noop_hi_def)
qed

lemma code_tbl_second_tag_zero_noop:
  assumes "code_tbl_matches t"
      and "unat (op :: 8 word) < 256"
      and "UCAST(8 \<rightarrow> 32) (code_tbl_'' t .[unat op] .[3]) = (0 :: 32 word)"
  shows "ity (snd (default_entry (unat op))) = NOOP"
proof -
  have tag0: "code_tbl_'' t .[unat op] .[3] = (0 :: 8 word)"
  proof -
    have "unat (UCAST(8 \<rightarrow> 32) (code_tbl_'' t .[unat op] .[3]) :: 32 word) = 0"
      using assms(3) by simp
    hence "unat (code_tbl_'' t .[unat op] .[3]) = 0"
      by (simp add: unat_ucast_upcast is_up)
    thus ?thesis by (simp add: unat_eq_0)
  qed
  have h_eq: "byte_to_hi (code_tbl_'' t .[unat op] .[3])
                   (code_tbl_'' t .[unat op] .[4])
                   (code_tbl_'' t .[unat op] .[5])
      = snd (default_entry (unat op))"
    by (rule code_tbl_matches_second_half[OF assms(1,2)])
  show ?thesis
    using h_eq[symmetric] tag0 by (simp add: byte_to_hi_def noop_hi_def)
qed

lemma code_tbl_first_tag_one_add:
  assumes "code_tbl_matches t"
      and "unat (op :: 8 word) < 256"
      and "UCAST(8 \<rightarrow> 32) (code_tbl_'' t .[unat op] .[0]) = (1 :: 32 word)"
  shows "ity (fst (default_entry (unat op))) = IADD"
proof -
  have tag1_eq:
    "UCAST(8 \<rightarrow> 32) (code_tbl_'' t .[unat op] .[0]) = (of_nat 1 :: 32 word)"
    using assms(3) by simp
  have tag1: "code_tbl_'' t .[unat op] .[0] = (of_nat 1 :: 8 word)"
    by (rule ucast8_32_eq_of_nat[OF tag1_eq]) simp
  have h_eq: "byte_to_hi (code_tbl_'' t .[unat op] .[0])
                   (code_tbl_'' t .[unat op] .[1])
                   (code_tbl_'' t .[unat op] .[2])
      = fst (default_entry (unat op))"
    by (rule code_tbl_matches_first_half[OF assms(1,2)])
  show ?thesis
    using h_eq[symmetric] tag1 by (simp add: byte_to_hi_def add_hi_def)
qed

lemma code_tbl_second_tag_one_add:
  assumes "code_tbl_matches t"
      and "unat (op :: 8 word) < 256"
      and "UCAST(8 \<rightarrow> 32) (code_tbl_'' t .[unat op] .[3]) = (1 :: 32 word)"
  shows "ity (snd (default_entry (unat op))) = IADD"
proof -
  have tag1_eq:
    "UCAST(8 \<rightarrow> 32) (code_tbl_'' t .[unat op] .[3]) = (of_nat 1 :: 32 word)"
    using assms(3) by simp
  have tag1: "code_tbl_'' t .[unat op] .[3] = (of_nat 1 :: 8 word)"
    by (rule ucast8_32_eq_of_nat[OF tag1_eq]) simp
  have h_eq: "byte_to_hi (code_tbl_'' t .[unat op] .[3])
                   (code_tbl_'' t .[unat op] .[4])
                   (code_tbl_'' t .[unat op] .[5])
...
lemma code_tbl_current_half_tag_one_add:
  ...
  shows "ity
          (if which = 0
           then fst (default_entry (unat op))
           else snd (default_entry (unat op))) = IADD"
proof -
  have which_cases: "which = 0 \<or> which = 1"
  proof -
    have "unat which = 0 \<or> unat which = 1"
  ...
  proof (rule disjE[OF which_cases])
    assume which0: "which = 0"
    have tag1_0:
      "UCAST(8 \<rightarrow> 32) (code_tbl_'' s.[unat op].[0]) = (1 :: 32 word)"
      using tag1 which0 by simp
    show ?thesis
      using code_tbl_first_tag_one_add[OF matches op_lt tag1_0] which0 by simp
  next
    assume which1: "which = 1"
    have tag1_3:
      "UCAST(8 \<rightarrow> 32) (code_tbl_'' s.[unat op].[3]) = (1 :: 32 word)"
      using tag1 which1 by simp
    show ?thesis
      using code_tbl_second_tag_one_add[OF matches op_lt tag1_3] which1 by simp
  qed
qed

lemma code_tbl_first_tag_two_run:
  assumes "code_tbl_matches t"
      and "unat (op :: 8 word) < 256"
      and "UCAST(8 \<rightarrow> 32) (code_tbl_'' t .[unat op] .[0]) = (2 :: 32 word)"
  shows "ity (fst (default_entry (unat op))) = IRUN"
proof -
  have tag2_eq:
    "UCAST(8 \<rightarrow> 32) (code_tbl_'' t .[unat op] .[0]) = (of_nat 2 :: 32 word)"
    using assms(3) by simp
  have tag2: "code_tbl_'' t .[unat op] .[0] = (of_nat 2 :: 8 word)"
    by (rule ucast8_32_eq_of_nat[OF tag2_eq]) simp
  have h_eq: "byte_to_hi (code_tbl_'' t .[unat op] .[0])
                   (code_tbl_'' t .[unat op] .[1])
                   (code_tbl_'' t .[unat op] .[2])
      = fst (default_entry (unat op))"
    by (rule code_tbl_matches_first_half[OF assms(1,2)])
  show ?thesis
    using h_eq[symmetric] tag2 by (simp add: byte_to_hi_def run_hi_def)
qed

lemma code_tbl_second_tag_two_run:
  assumes "code_tbl_matches t"
      and "unat (op :: 8 word) < 256"
      and "UCAST(8 \<rightarrow> 32) (code_tbl_'' t .[unat op] .[3]) = (2 :: 32 word)"
  shows "ity (snd (default_entry (unat op))) = IRUN"
proof -
  have tag2_eq:
    "UCAST(8 \<rightarrow> 32) (code_tbl_'' t .[unat op] .[3]) = (of_nat 2 :: 32 word)"
    using assms(3) by simp
  have tag2: "code_tbl_'' t .[unat op] .[3] = (of_nat 2 :: 8 word)"
    by (rule ucast8_32_eq_of_nat[OF tag2_eq]) simp
  have h_eq: "byte_to_hi (code_tbl_'' t .[unat op] .[3])
                   (code_tbl_'' t .[unat op] .[4])
                   (code_tbl_'' t .[unat op] .[5])
...
lemma code_tbl_current_half_tag_two_run:
  fixes which :: "32 word" and op :: "8 word"
  assumes matches: "code_tbl_matches s"
      and op_lt: "unat op < 256"
      and which_lt: "unat which < 2"
  ...
  shows "ity
          (if which = 0
           then fst (default_entry (unat op))
           else snd (default_entry (unat op))) = IRUN"
proof -
  have which_cases: "which = 0 \<or> which = 1"
  proof -
    have "unat which = 0 \<or> unat which = 1"
  ...
  proof (rule disjE[OF which_cases])
    assume which0: "which = 0"
    have tag2_0:
      "UCAST(8 \<rightarrow> 32) (code_tbl_'' s.[unat op].[0]) = (2 :: 32 word)"
      using tag2 which0 by simp
    show ?thesis
      using code_tbl_first_tag_two_run[OF matches op_lt tag2_0] which0 by simp
  next
    assume which1: "which = 1"
    have tag2_3:
      "UCAST(8 \<rightarrow> 32) (code_tbl_'' s.[unat op].[3]) = (2 :: 32 word)"
      using tag2 which1 by simp
    show ?thesis
      using code_tbl_second_tag_two_run[OF matches op_lt tag2_3] which1 by simp
  qed
qed

lemma default_entry_first_copy_mode_lt9:
  assumes "op < 256"
      and "ity (fst (default_entry op)) = ICOPY mode"
  shows "mode < 9"
  using assms
  by (auto simp: default_entry_def Let_def noop_hi_def add_hi_def
      run_hi_def copy_hi_def split: if_splits)

lemma default_entry_second_copy_mode_lt9:
  assumes "op < 256"
      and "ity (snd (default_entry op)) = ICOPY mode"
  shows "mode < 9"
  using assms
  by (auto simp: default_entry_def Let_def noop_hi_def add_hi_def
      run_hi_def copy_hi_def split: if_splits)

lemma code_tbl_current_half_copy_mode_lt9:
  fixes which :: "32 word" and op :: "8 word"
  assumes matches: "code_tbl_matches s"
  ...
  shows "unat
          (UCAST(8 \<rightarrow> 32)
            (code_tbl_'' s.[unat op].[unat (which * (3 :: 32 word) + 2)])
           :: 32 word) < 9"
proof -
  have which_cases: "which = 0 \<or> which = 1"
  proof -
  ...
  proof (rule disjE[OF which_cases])
    ...
    have tag3: "?tag = (3 :: 8 word)"
      using tag_cases tag_nz tag_ne1 tag_ne2 which0 by auto
    have h_eq:
      "byte_to_hi ?tag (code_tbl_'' s.[unat op].[1]) ?mode =
       fst (default_entry (unat op))"
      by (rule code_tbl_matches_first_half[OF matches op_lt])
    have copy_mode:
      "ity (fst (default_entry (unat op))) = ICOPY (unat ?mode)"
      using h_eq[symmetric] tag3
      by (simp add: byte_to_hi_def copy_hi_def)
    have "unat ?mode < 9"
      by (rule default_entry_first_copy_mode_lt9[OF op_lt copy_mode])
    thus ?thesis
      using which0 unat_lt2p[of ?mode]
  ...
  next
    ...
    have tag_cases: "?tag = 0 \<or> ?tag = 1 \<or> ?tag = 2 \<or> ?tag = 3"
      by (rule word8_unat_le3_cases[OF tag_le3])
    have tag3: "?tag = (3 :: 8 word)"
      using tag_cases tag_nz tag_ne1 tag_ne2 which1 by auto
    have h_eq:
      "byte_to_hi ?tag (code_tbl_'' s.[unat op].[4]) ?mode =
       snd (default_entry (unat op))"
      by (rule code_tbl_matches_second_half[OF matches op_lt])
    have copy_mode:
      "ity (snd (default_entry (unat op))) = ICOPY (unat ?mode)"
      using h_eq[symmetric] tag3
      by (simp add: byte_to_hi_def copy_hi_def)
    have "unat ?mode < 9"
      by (rule default_entry_second_copy_mode_lt9[OF op_lt copy_mode])
    thus ?thesis
      using which1 unat_lt2p[of ?mode]
      by (simp add: unat_ucast)
  qed
qed

lemma code_tbl_current_half_tag_three_copy:
  fixes which :: "32 word" and op :: "8 word"
  assumes matches: "code_tbl_matches s"
      and tags_valid: "code_tbl_tags_valid s"
      and op_lt: "unat op < 256"
...
proof -
  ...
  proof (rule disjE[OF which_cases])
    ...
    have h_eq:
      ...
      by (rule code_tbl_matches_first_half[OF matches op_lt])
    show ?thesis
      using h_eq[symmetric] tag3 which0 unat_lt2p[of ?mode]
      by (simp add: byte_to_hi_def copy_hi_def unat_ucast)
  next
    assume which1: "which = 1"
    let ?tag = "code_tbl_'' s.[unat op].[3]"
    let ?mode = "code_tbl_'' s.[unat op].[5]"
    ...
    have h_eq:
      ...
      by (rule code_tbl_matches_second_half[OF matches op_lt])
    show ?thesis
      using h_eq[symmetric] tag3 which1 unat_lt2p[of ?mode]
      by (simp add: byte_to_hi_def copy_hi_def unat_ucast)
  qed
qed

lemma default_entry_first_not_noop:
  assumes "op < 256"
  shows "ity (fst (default_entry op)) \<noteq> NOOP"
  using assms
  by (auto simp: default_entry_def Let_def
      add_hi_def run_hi_def copy_hi_def noop_hi_def)

lemma code_tbl_current_half_varint_nonnoop:
  fixes which :: "32 word" and op :: "8 word"
  assumes matches: "code_tbl_matches s"
      and tags_valid: "code_tbl_tags_valid s"
      and op_lt: "unat op < 256"
...
proof -
  ...
  proof (rule disjE[OF which_cases])
    assume which0: "which = 0"
    have sz0_32:
      "UCAST(8 \<rightarrow> 32) (code_tbl_'' s.[unat op].[1]) = (0 :: 32 word)"
      using size0 which0 by simp
    have sz0: "code_tbl_'' s.[unat op].[1] = (0 :: 8 word)"
    proof -
      have unat_uc:
        "unat (UCAST(8 \<rightarrow> 32) (code_tbl_'' s.[unat op].[1]) :: 32 word) =
         unat (code_tbl_'' s.[unat op].[1])"
        using unat_lt2p[of "code_tbl_'' s.[unat op].[1]"]
        by (simp add: unat_ucast)
      have "unat (code_tbl_'' s.[unat op].[1]) = 0"
        using sz0_32 unat_uc by simp
      thus ?thesis by (simp add: unat_eq_0)
    qed
    have h_eq:
      "byte_to_hi (code_tbl_'' s.[unat op].[0])
        (code_tbl_'' s.[unat op].[1])
        (code_tbl_'' s.[unat op].[2])
       = fst (default_entry (unat op))"
      by (rule code_tbl_matches_first_half[OF matches op_lt])
    show ?thesis
      using h_def h_eq[symmetric] sz0 which0
      by (auto simp: byte_to_hi_def noop_hi_def add_hi_def
          run_hi_def copy_hi_def split: if_splits)
  next
    assume which1: "which = 1"
    have sz0_32:
      "UCAST(8 \<rightarrow> 32) (code_tbl_'' s.[unat op].[4]) = (0 :: 32 word)"
      using size0 which1 by simp
    have sz0: "code_tbl_'' s.[unat op].[4] = (0 :: 8 word)"
    proof -
      have unat_uc:
        "unat (UCAST(8 \<rightarrow> 32) (code_tbl_'' s.[unat op].[4]) :: 32 word) =
         unat (code_tbl_'' s.[unat op].[4])"
        using unat_lt2p[of "code_tbl_'' s.[unat op].[4]"]
        by (simp add: unat_ucast)
      have "unat (code_tbl_'' s.[unat op].[4]) = 0"
        using sz0_32 unat_uc by simp
      thus ?thesis by (simp add: unat_eq_0)
    qed
    have h_eq:
      "byte_to_hi (code_tbl_'' s.[unat op].[3])
        (code_tbl_'' s.[unat op].[4])
        (code_tbl_'' s.[unat op].[5])
       = snd (default_entry (unat op))"
      by (rule code_tbl_matches_second_half[OF matches op_lt])
    show ?thesis
      using h_def h_eq[symmetric] sz0 which1
      by (auto simp: byte_to_hi_def noop_hi_def add_hi_def
          run_hi_def copy_hi_def split: if_splits)
  qed
  moreover have h_not_noop: "ity h \<noteq> NOOP"
  proof (rule disjE[OF which_cases])
    assume which0: "which = 0"
    show ?thesis
  ...
  next
    assume which1: "which = 1"
    have tag_nz_3: "code_tbl_'' s.[unat op].[3] \<noteq> (0 :: 8 word)"
    proof
      assume tag0: "code_tbl_'' s.[unat op].[3] = (0 :: 8 word)"
      have "UCAST(8 \<rightarrow> 32) (code_tbl_'' s.[unat op].[3]) = (0 :: 32 word)"
        using tag0 by simp
      thus False using tag_nz which1 by simp
    qed
    have h_eq:
      "byte_to_hi (code_tbl_'' s.[unat op].[3])
        (code_tbl_'' s.[unat op].[4])
       ...
       = snd (default_entry (unat op))"
      by (rule code_tbl_matches_second_half[OF matches op_lt])
    have tag_le3: "unat (code_tbl_'' s.[unat op].[3]) \<le> 3"
      using tags_valid op_lt unfolding code_tbl_tags_valid_def by simp
    have tag_cases:
      "code_tbl_'' s.[unat op].[3] = 0 \<or>
       code_tbl_'' s.[unat op].[3] = 1 \<or>
       code_tbl_'' s.[unat op].[3] = 2 \<or>
       code_tbl_'' s.[unat op].[3] = 3"
      by (rule word8_unat_le3_cases[OF tag_le3])
    show ?thesis
      using h_def h_eq[symmetric] tag_nz_3 tag_cases which1
      by (auto simp: byte_to_hi_def noop_hi_def add_hi_def
          run_hi_def copy_hi_def split: if_splits)
  qed
  ultimately show "isz h = 0" "ity h \<noteq> NOOP"
    by simp_all
qed
...
lemma add_loop_correct_core:
  fixes sz :: "32 word" and data_cursor :: "32 word"
    and inst_cursor :: "32 word" and addr_cursor :: "32 word"
    and tgt_pos :: "32 word" and np :: "32 word"
    and patch :: "8 word ptr" and out :: "8 word ptr"
  assumes core: "decode_loop_inv_core s0 patch patch_n src src_n out
                  src_seg_off src_seg_len tgt_len
                  data_end inst_end addr_end src_seg
...
proof -
  ...
  note invD = decode_loop_invD[OF inv]
  have no_overflow_data: "unat data_cursor + unat sz < 2 ^ 32"
    by (rule inv_no_overflow_data[OF inv sz_fits_data])
  have no_overflow_tgt: "unat tgt_pos + unat sz < 2 ^ 32"
    by (rule inv_no_overflow_tgt[OF inv sz_fits_tgt])
  have data_in_range: "unat data_cursor + unat sz \<le> patch_n"
    using sz_fits_data invD(6) invD(10) by (simp add: word_le_nat_alt)
  have data_valid:
    "\<forall>j < unat sz.
       ptr_valid (heap_typing s) (patch +\<^sub>p uint (data_cursor + of_nat j))"
  proof (intro allI impI)
    ...
    have idx_unat:
      "unat (data_cursor + of_nat j :: 32 word) = unat data_cursor + j"
      by (rule unat_add_of_nat_index[OF j_lt no_overflow_data])
    have idx_lt: "unat (data_cursor + of_nat j :: 32 word) < patch_n"
      using j_lt data_in_range idx_unat by simp
    show "ptr_valid (heap_typing s) (patch +\<^sub>p uint (data_cursor + of_nat j))"
      by (rule buf_valid_uintD[OF invD(3) idx_lt])
  qed
  have out_valid:
    "\<forall>j < unat sz.
       ptr_valid (heap_typing s) (out +\<^sub>p uint (tgt_pos + of_nat j))"
  ...
  proof (intro allI impI)
    ...
    show "out +\<^sub>p uint (tgt_pos + of_nat i) \<noteq> patch +\<^sub>p int j"
      by (subst lhs_eq, rule neq)
  qed
  have out_inj:
    "\<forall>i < unat sz. \<forall>j < unat sz.
       i \<noteq> j \<longrightarrow>
       out +\<^sub>p uint (tgt_pos + of_nat i) \<noteq> out +\<^sub>p uint (tgt_pos + of_nat j)"
  proof (intro allI impI)
    fix i j
    assume i_lt: "i < unat sz" and j_lt: "j < unat sz" and ij: "i \<noteq> j"
    have idx_i:
      "unat (tgt_pos + of_nat i :: 32 word) = unat tgt_pos + i"
      by (rule unat_add_of_nat_index[OF i_lt no_overflow_tgt])
    have idx_j:
      "unat (tgt_pos + of_nat j :: 32 word) = unat tgt_pos + j"
      by (rule unat_add_of_nat_index[OF j_lt no_overflow_tgt])
    have idx_i_lt: "unat (tgt_pos + of_nat i :: 32 word) < tgt_len"
      using i_lt sz_fits_tgt idx_i by simp
    have idx_j_lt: "unat (tgt_pos + of_nat j :: 32 word) < tgt_len"
      using j_lt sz_fits_tgt idx_j by simp
    have idx_ne:
      "unat (tgt_pos + of_nat i :: 32 word) \<noteq>
       unat (tgt_pos + of_nat j :: 32 word)"
      using idx_i idx_j ij by simp
    have lhs_eq:
      "out +\<^sub>p uint (tgt_pos + of_nat i) =
       out +\<^sub>p int (unat (tgt_pos + of_nat i :: 32 word))"
      by (simp only: uint_nat)
    have rhs_eq:
      "out +\<^sub>p uint (tgt_pos + of_nat j) =
       out +\<^sub>p int (unat (tgt_pos + of_nat j :: 32 word))"
      by (simp only: uint_nat)
    have neq:
      "out +\<^sub>p int (unat (tgt_pos + of_nat i :: 32 word)) \<noteq>
       out +\<^sub>p int (unat (tgt_pos + of_nat j :: 32 word))"
      by (rule invD(17)[rule_format, OF idx_i_lt idx_j_lt idx_ne])
    show "out +\<^sub>p uint (tgt_pos + of_nat i) \<noteq>
          out +\<^sub>p uint (tgt_pos + of_nat j)"
      by (subst lhs_eq, subst rhs_eq, rule neq)
  qed
  have src_disj:
    "\<forall>i < src_n. \<forall>j < unat sz.
       src +\<^sub>p int i \<noteq> out +\<^sub>p uint (tgt_pos + of_nat j)"
  proof (intro allI impI)
    fix i j
    assume i_lt: "i < src_n" and j_lt: "j < unat sz"
    have idx_unat:
      "unat (tgt_pos + of_nat j :: 32 word) = unat tgt_pos + j"
      by (rule unat_add_of_nat_index[OF j_lt no_overflow_tgt])
    have idx_lt: "unat (tgt_pos + of_nat j :: 32 word) < tgt_len"
    ...
    have out_eq:
      ...
      by (simp only: uint_nat)
    have neq:
      "out +\<^sub>p int (unat (tgt_pos + of_nat j :: 32 word)) \<noteq>
       src +\<^sub>p int i"
      by (rule invD(16)[rule_format, OF idx_lt i_lt])
    show "src +\<^sub>p int i \<noteq> out +\<^sub>p uint (tgt_pos + of_nat j)"
      using neq by (subst out_eq) simp
  qed
  have out_prefix_disj:
    "\<forall>i < unat tgt_pos. \<forall>j < unat sz.
       out +\<^sub>p int i \<noteq> out +\<^sub>p uint (tgt_pos + of_nat j)"
  proof (intro allI impI)
    fix i j
    assume i_lt: "i < unat tgt_pos" and j_lt: "j < unat sz"
    have idx_unat:
      "unat (tgt_pos + of_nat j :: 32 word) = unat tgt_pos + j"
      by (rule unat_add_of_nat_index[OF j_lt no_overflow_tgt])
    have idx_lt: "unat (tgt_pos + of_nat j :: 32 word) < tgt_len"
    ...
    have i_tgt: "i < tgt_len"
      using i_lt invD(9) by simp
    have idx_ne: "i \<noteq> unat (tgt_pos + of_nat j :: 32 word)"
      using i_lt idx_unat by simp
    have out_eq:
      "out +\<^sub>p uint (tgt_pos + of_nat j) =
       out +\<^sub>p int (unat (tgt_pos + of_nat j :: 32 word))"
      by (simp only: uint_nat)
    have neq:
      "out +\<^sub>p int i \<noteq>
       out +\<^sub>p int (unat (tgt_pos + of_nat j :: 32 word))"
      by (rule invD(17)[rule_format, OF i_tgt idx_lt idx_ne])
    show "out +\<^sub>p int i \<noteq> out +\<^sub>p uint (tgt_pos + of_nat j)"
      by (subst out_eq, rule neq)
  qed
  show ?thesis
    by (rule add_loop_correct_c
      [OF data_valid out_valid disj src_disj out_prefix_disj out_inj
          no_overflow_data no_overflow_tgt data_in_range])
qed

lemma add_loop_correct_core_ex:
  fixes sz :: "32 word" and data_cursor :: "32 word"
    and tgt_pos :: "32 word"
    and patch :: "8 word ptr" and out :: "8 word ptr"
  assumes pre:
    "\<exists>(data_end :: 32 word) (inst_end :: 32 word) (addr_end :: 32 word)
       (src_seg :: 8 word list) (inst_cursor :: 32 word)
       (addr_cursor :: 32 word) (np :: 32 word) (dst :: dec_state) (c :: cache).
       decode_loop_inv_core s0 patch patch_n src src_n out
         src_seg_off src_seg_len tgt_len
         data_end inst_end addr_end src_seg
         data_cursor inst_cursor addr_cursor tgt_pos np dst c s \<and>
       unat sz \<le> unat data_end - unat data_cursor \<and>
       unat tgt_pos + unat sz \<le> tgt_len"
  shows "(whileLoop (\<lambda>(j :: 32 word) st. j < sz)
           (\<lambda>j. do {
              guard (\<lambda>st. ptr_valid (heap_typing st) (out +\<^sub>p uint (tgt_pos + j)));
              guard (\<lambda>st. ptr_valid (heap_typing st) (patch +\<^sub>p uint (data_cursor + j)));
         ...
         \<lbrace> \<lambda>r t. r = Result sz \<and>
            ...
            code_tbl_'' t = code_tbl_'' s \<rbrace>"
proof -
  obtain data_end inst_end addr_end src_seg inst_cursor addr_cursor np dst c where
    core:
      "decode_loop_inv_core s0 patch patch_n src src_n out
        src_seg_off src_seg_len tgt_len
        data_end inst_end addr_end src_seg
        data_cursor inst_cursor addr_cursor tgt_pos np dst c s"
    and data_fit: "unat sz \<le> unat data_end - unat data_cursor"
    and tgt_fit: "unat tgt_pos + unat sz \<le> tgt_len"
    using pre by blast
  show ?thesis
    by (rule add_loop_correct_core[OF core data_fit tgt_fit])
qed

lemma run_loop_correct_core:
  fixes sz :: "32 word" and data_cursor :: "32 word"
    and inst_cursor :: "32 word" and addr_cursor :: "32 word"
    and tgt_pos :: "32 word" and np :: "32 word"
    and patch :: "8 word ptr" and out :: "8 word ptr"
...
proof -
  ...
  proof (intro allI impI)
    ...
    have neq:
	  ...
	  qed
	  show ?thesis
	    by (rule run_loop_correct
	      [OF out_valid disj src_disj out_prefix_disj out_inj no_overflow_tgt])
	qed

lemma run_loop_correct_core_ex:
  fixes sz :: "32 word" and data_cursor :: "32 word"
    and tgt_pos :: "32 word"
    and patch :: "8 word ptr" and out :: "8 word ptr"
  assumes pre:
...
lemma decode_loop_inv_core_after_add_exec:
  fixes sz :: "32 word"
  assumes core:
    "decode_loop_inv_core s0 patch patch_n src src_n out
      src_seg_off src_seg_len tgt_len
      data_end inst_end addr_end src_seg
      data_cursor inst_cursor addr_cursor tgt_pos np dst c t"
    and h_add: "ity h = IADD"
    and exec:
      "exec_half h (unat sz) src_seg (unat src_seg_len) tgt_len dst = Inl dst'"
    and sz_fits_data: "unat sz \<le> unat data_end - unat data_cursor"
    and sz_fits_tgt: "unat tgt_pos + unat sz \<le> tgt_len"
    and no_overflow_data: "unat data_cursor + unat sz < 2 ^ 32"
    and no_overflow_tgt: "unat tgt_pos + unat sz < 2 ^ 32"
  ...
  shows
    "decode_loop_inv_core s0 patch patch_n src src_n out
      src_seg_off src_seg_len tgt_len
      data_end inst_end addr_end src_seg
      (data_cursor + sz) inst_cursor addr_cursor (tgt_pos + sz) np dst' c t'"
proof -
  have inv:
    "decode_loop_inv s0 patch patch_n src src_n out
      src_seg_off src_seg_len tgt_len
  ...
  have dst_addr:
    "ds_addr_rem dst =
      drop (unat addr_cursor) (take (unat addr_end) (heap_bytes s0 patch patch_n))"
    using core unfolding decode_loop_inv_core_def by simp
  have dst_tgt: "ds_tgt dst = heap_bytes t out (unat tgt_pos)"
    using core unfolding decode_loop_inv_core_def by simp
  have dst_cache: "ds_cache dst = c"
    using core unfolding decode_loop_inv_core_def by simp
  have cache_abs_t: "cache_abs t c np"
    using core unfolding decode_loop_inv_core_def by simp
  have cache_wf_c: "cache_wf c"
    using core unfolding decode_loop_inv_core_def by simp
  have data_exec: "ds_data_rem dst' = drop (unat sz) (ds_data_rem dst)"
    by (rule exec_half_add_conditions(3)[OF h_add exec])
  have tgt_exec: "ds_tgt dst' = ds_tgt dst @ take (unat sz) (ds_data_rem dst)"
    by (rule exec_half_add_conditions(4)[OF h_add exec])
  have inst_exec: "ds_inst_rem dst' = ds_inst_rem dst"
    by (rule exec_half_add_conditions(5)[OF h_add exec])
  have addr_exec: "ds_addr_rem dst' = ds_addr_rem dst"
    by (rule exec_half_add_conditions(6)[OF h_add exec])
  have cache_exec: "ds_cache dst' = ds_cache dst"
    by (rule exec_half_add_conditions(7)[OF h_add exec])
  have unat_dc_sz:
    "unat (data_cursor + sz) = unat data_cursor + unat sz"
    using no_overflow_data by (simp add: unat_word_ariths(1))
  have unat_tp_sz:
    "unat (tgt_pos + sz) = unat tgt_pos + unat sz"
    using no_overflow_tgt by (simp add: unat_word_ariths(1))
  have new_data_rem:
    "drop (unat (data_cursor + sz))
       (take (unat data_end) (heap_bytes s0 patch patch_n)) =
     drop (unat sz) (ds_data_rem dst)"
  proof -
    let ?xs = "take (unat data_end) (heap_bytes s0 patch patch_n)"
    have "drop (unat sz) (drop (unat data_cursor) ?xs) =
          drop (unat sz + unat data_cursor) ?xs"
      by (rule drop_drop)
    hence "drop (unat data_cursor + unat sz) ?xs =
           drop (unat sz) (drop (unat data_cursor) ?xs)"
      by (simp add: add.commute)
    thus ?thesis
      using unat_dc_sz dst_data by simp
  qed
  have new_tgt:
    "heap_bytes t' out (unat tgt_pos + unat sz) =
     ds_tgt dst @ take (unat sz) (ds_data_rem dst)"
  proof (rule nth_equalityI)
    show "length (heap_bytes t' out (unat tgt_pos + unat sz)) =
          length (ds_tgt dst @ take (unat sz) (ds_data_rem dst))"
    proof -
      have "length (ds_tgt dst) = unat tgt_pos"
        using dst_tgt by simp
      moreover have "length (take (unat sz) (ds_data_rem dst)) = unat sz"
      proof -
        have "length (ds_data_rem dst) = unat data_end - unat data_cursor"
          using dst_data invD(10) invD(6) by (simp add: word_le_nat_alt)
        thus ?thesis
          using sz_fits_data by simp
      qed
      ultimately show ?thesis by simp
    qed
  next
    fix i
    assume i_bound: "i < length (heap_bytes t' out (unat tgt_pos + unat sz))"
    hence i_lt: "i < unat tgt_pos + unat sz" by simp
    show "heap_bytes t' out (unat tgt_pos + unat sz) ! i =
          (ds_tgt dst @ take (unat sz) (ds_data_rem dst)) ! i"
    proof (cases "i < unat tgt_pos")
      case True
      have lhs:
        "heap_bytes t' out (unat tgt_pos + unat sz) ! i =
         heap_w8 t' (out +\<^sub>p int i)"
        using i_lt by (simp add: heap_bytes_nth)
      have "heap_w8 t' (out +\<^sub>p int i) = heap_w8 t (out +\<^sub>p int i)"
        using out_prefix_preserved True by auto
      also have "... = heap_bytes t out (unat tgt_pos) ! i"
        using True by (simp add: heap_bytes_nth)
      also have "... = ds_tgt dst ! i"
        using dst_tgt by simp
      finally have "heap_bytes t' out (unat tgt_pos + unat sz) ! i =
        ds_tgt dst ! i"
        using lhs by simp
      moreover have "(ds_tgt dst @ take (unat sz) (ds_data_rem dst)) ! i =
        ds_tgt dst ! i"
        using True dst_tgt by (simp add: nth_append)
      ultimately show ?thesis by simp
    next
      ...
      have rhs:
        ...
        using i_ge dst_tgt by (simp add: nth_append)
      have data_rem_nth:
        "(take (unat sz) (ds_data_rem dst)) ! ?j = ds_data_rem dst ! ?j"
        using j_lt by simp
      have "ds_data_rem dst ! ?j =
            (drop (unat data_cursor)
              (take (unat data_end) (heap_bytes s0 patch patch_n))) ! ?j"
        using dst_data by simp
      also have "... =
        (take (unat data_end) (heap_bytes s0 patch patch_n)) !
          (unat data_cursor + ?j)"
      proof -
        have "unat data_cursor + ?j < unat data_end"
          using sz_fits_data j_lt invD(6) by (simp add: word_le_nat_alt)
        moreover have
          "length (take (unat data_end) (heap_bytes s0 patch patch_n)) =
           unat data_end"
          using invD(10) by simp
        ultimately show ?thesis by (simp add: nth_drop)
      qed
      ...
      proof -
        ...
        thus ?thesis by (simp add: heap_bytes_nth)
      qed
      finally have data_byte:
        "ds_data_rem dst ! ?j =
         heap_w8 s0 (patch +\<^sub>p int (unat data_cursor + ?j))" .
      have patch_ptr_eq:
        "patch +\<^sub>p int (unat data_cursor + ?j) =
         patch +\<^sub>p uint (data_cursor + of_nat ?j)"
      proof -
        have unat_eq:
          "unat (data_cursor + of_nat ?j :: 32 word) = unat data_cursor + ?j"
        proof -
          ...
          thus ?thesis by (simp add: unat_word_ariths(1) unat_of_nat)
        qed
        have "uint (data_cursor + of_nat ?j :: 32 word) =
              int (unat data_cursor + ?j)"
          by (simp only: uint_nat unat_eq)
        thus ?thesis by simp
      qed
      show ?thesis
        using lhs ptr_eq add_result[rule_format, OF j_lt]
          rhs data_rem_nth data_byte patch_ptr_eq
        by simp
    qed
  qed
  have cache_abs': "cache_abs t' c np"
  ...
  have inv_after:
    "decode_loop_inv s0 patch patch_n src src_n out
      ...
      data_end inst_end addr_end src_seg
      (data_cursor + sz) inst_cursor addr_cursor (tgt_pos + sz) np t'"
    by (rule decode_loop_inv_after_add
      [OF inv sz_fits_data sz_fits_tgt no_overflow_data no_overflow_tgt
          patch_preserved src_preserved add_result out_prefix_preserved
	          typing_preserved cache_preserved code_tbl_preserved])
    ...
    unfolding decode_loop_inv_core_def
    apply (intro conjI)
    using inst_exec dst_inst apply simp
    using data_exec new_data_rem apply simp
    using addr_exec dst_addr apply simp
    using tgt_exec new_tgt unat_tp_sz apply simp
    using cache_exec dst_cache apply simp
    using cache_abs' apply simp
...
lemma decode_loop_inv_core_after_run_exec:
  fixes sz :: "32 word" and fill :: "8 word"
  assumes core:
    "decode_loop_inv_core s0 patch patch_n src src_n out
      src_seg_off src_seg_len tgt_len
    ...
    and src_preserved:
      "\<forall>j < src_n. heap_w8 t' (src +\<^sub>p int j) = heap_w8 s0 (src +\<^sub>p int j)"
    and run_result:
      "\<forall>j < unat sz. heap_w8 t' (out +\<^sub>p uint (tgt_pos + of_nat j)) = fill"
    and fill_eq: "fill = heap_w8 s0 (patch +\<^sub>p int (unat data_cursor))"
    and out_prefix_preserved:
      "\<forall>i < unat tgt_pos. heap_w8 t' (out +\<^sub>p int i) = heap_w8 t (out +\<^sub>p int i)"
    and typing_preserved: "heap_typing t' = heap_typing t"
  ...
  shows
    "decode_loop_inv_core s0 patch patch_n src src_n out
      src_seg_off src_seg_len tgt_len
      data_end inst_end addr_end src_seg
      (data_cursor + 1) inst_cursor addr_cursor (tgt_pos + sz) np dst' c t'"
proof -
  have inv:
    "decode_loop_inv s0 patch patch_n src src_n out
      src_seg_off src_seg_len tgt_len
  ...
  have cache_wf_c: "cache_wf c"
    using core unfolding decode_loop_inv_core_def by simp
  have data_exec: "ds_data_rem dst' = tl (ds_data_rem dst)"
    by (rule exec_half_run_conditions(3)[OF h_run exec])
  have tgt_exec:
    "ds_tgt dst' = ds_tgt dst @ replicate (unat sz) (hd (ds_data_rem dst))"
    by (rule exec_half_run_conditions(4)[OF h_run exec])
  have inst_exec: "ds_inst_rem dst' = ds_inst_rem dst"
    by (rule exec_half_run_conditions(5)[OF h_run exec])
  have addr_exec: "ds_addr_rem dst' = ds_addr_rem dst"
    by (rule exec_half_run_conditions(6)[OF h_run exec])
  have cache_exec: "ds_cache dst' = ds_cache dst"
    by (rule exec_half_run_conditions(7)[OF h_run exec])
  have unat_dc1:
    "unat (data_cursor + 1 :: 32 word) = unat data_cursor + 1"
    using no_overflow_data by (simp add: unat_word_ariths(1))
  have unat_tp_sz:
    "unat (tgt_pos + sz) = unat tgt_pos + unat sz"
    using no_overflow_tgt by (simp add: unat_word_ariths(1))
  have data_rem_len:
    "length (ds_data_rem dst) = unat data_end - unat data_cursor"
    using dst_data invD(10) invD(6) by (simp add: word_le_nat_alt)
  have data_rem_nonempty: "0 < length (ds_data_rem dst)"
    using fill_read data_rem_len invD(6) by (simp add: word_le_nat_alt)
  have fill_is_hd: "fill = hd (ds_data_rem dst)"
  proof -
    have "hd (ds_data_rem dst) = ds_data_rem dst ! 0"
      using data_rem_nonempty by (simp add: hd_conv_nth)
    also have "... =
      (drop (unat data_cursor) (take (unat data_end) (heap_bytes s0 patch patch_n))) ! 0"
      using dst_data by simp
    also have "... =
      (take (unat data_end) (heap_bytes s0 patch patch_n)) ! unat data_cursor"
    proof -
      have "unat data_cursor < length (take (unat data_end) (heap_bytes s0 patch patch_n))"
        using fill_read invD(10) by simp
      thus ?thesis by (simp add: nth_drop)
    ...
    also have "... = heap_w8 s0 (patch +\<^sub>p int (unat data_cursor))"
    proof -
      have "unat data_cursor < patch_n"
        using fill_read invD(10) by simp
      thus ?thesis by (simp add: heap_bytes_nth)
    qed
    finally show ?thesis
      using fill_eq by simp
  qed
  have new_data_rem:
    "drop (unat (data_cursor + 1))
       (take (unat data_end) (heap_bytes s0 patch patch_n)) =
     tl (ds_data_rem dst)"
  proof -
    let ?xs = "take (unat data_end) (heap_bytes s0 patch patch_n)"
    have eq1: "unat (data_cursor + 1 :: 32 word) = Suc (unat data_cursor)"
      using unat_dc1 by simp
    have "drop (Suc (unat data_cursor)) ?xs = tl (drop (unat data_cursor) ?xs)"
      by (simp add: drop_Suc tl_drop)
    thus ?thesis
      using eq1 dst_data by simp
  qed
  have new_tgt:
    "heap_bytes t' out (unat tgt_pos + unat sz) =
     ds_tgt dst @ replicate (unat sz) fill"
  ...
  next
    ...
    proof (cases "i < unat tgt_pos")
      case True
      have "heap_bytes t' out (unat tgt_pos + unat sz) ! i =
            heap_w8 t' (out +\<^sub>p int i)"
        using i_lt by (simp add: heap_bytes_nth)
      also have "... = heap_w8 t (out +\<^sub>p int i)"
        using out_prefix_preserved True by auto
    ...
    next
      ...
      qed
      also have "... = fill"
        using run_result[rule_format, OF j_lt] by simp
      finally have lhs:
        "heap_bytes t' out (unat tgt_pos + unat sz) ! i = fill" .
      have "(ds_tgt dst @ replicate (unat sz) fill) ! i =
            replicate (unat sz) fill ! ?j"
        using i_ge dst_tgt by (simp add: nth_append)
      also have "... = fill"
        using j_lt by simp
      finally show ?thesis
  ...
  have inv_after:
    "decode_loop_inv s0 patch patch_n src src_n out
      ...
      data_end inst_end addr_end src_seg
      (data_cursor + 1) inst_cursor addr_cursor (tgt_pos + sz) np t'"
    by (rule decode_loop_inv_after_run
      [OF inv fill_read sz_fits_tgt no_overflow_tgt no_overflow_data
          patch_preserved src_preserved run_result fill_eq out_prefix_preserved
	          typing_preserved cache_preserved code_tbl_preserved])
	  note afterD = decode_loop_invD[OF inv_after]
	  have after_src_tgt_fit: "unat src_seg_len + tgt_len < 2 ^ 32"
	    using inv_after unfolding decode_loop_inv_def by auto
    ...
    using data_exec new_data_rem apply simp
    using addr_exec dst_addr apply simp
    using tgt_exec new_tgt fill_is_hd unat_tp_sz apply simp
    using cache_exec dst_cache apply simp
    using cache_abs' apply simp
    using cache_wf_c apply simp
    using afterD(1) apply simp
...
lemma decode_one_prefix_run_data_nonempty:
  fixes which :: "32 word"
  assumes pop: "pop_byte (ds_inst_rem dst0) = Some (op, irest)"
      and prefix: "decode_one_prefix src_seg src_seg_len tgt_len dst0 (unat which) st"
      and whole: "decode_one src_seg src_seg_len tgt_len dst0 = Inl dst_final"
...
proof -
  ...
  proof (rule disjE[OF which_cases])
    ...
    have st_eq: "st = ?st1"
      using prefix pop which0
      by (simp add: decode_one_prefix_def Let_def)
    obtain sz1 irest1 st3 where exec1:
      "exec_half ?h1 sz1 src_seg src_seg_len tgt_len
        (?st1\<lparr>ds_inst_rem := irest1\<rparr>) = Inl st3"
      using whole pop
      by (auto simp: decode_one_def Let_def split: option.splits prod.splits sum.splits)
    have "ds_data_rem (?st1\<lparr>ds_inst_rem := irest1\<rparr>) \<noteq> []"
      by (rule exec_half_run_conditions(1)[OF run_first[OF which0] exec1])
    thus ?thesis
      using st_eq by simp
  next
    assume which1: "which = 1"
    ...
    obtain sz1 irest1 where
      ...
      using prefix pop which1
      by (auto simp: decode_one_prefix_def Let_def split: option.splits)
    obtain sz2 irest2 where exec2:
      "exec_half ?h2 sz2 src_seg src_seg_len tgt_len
        (st\<lparr>ds_inst_rem := irest2\<rparr>) = Inl dst_final"
      using whole pop resolve1 exec1
      by (auto simp: decode_one_def Let_def split: option.splits prod.splits sum.splits)
    have "ds_data_rem (st\<lparr>ds_inst_rem := irest2\<rparr>) \<noteq> []"
      by (rule exec_half_run_conditions(1)[OF run_second[OF which1] exec2])
    thus ?thesis by simp
  qed
qed
...
lemma decode_inner_inv_core_copy_varint_address:
  fixes which :: "32 word" and op :: "8 word"
  assumes which_lt: "unat which < 2"
      and inner_inv:
        "decode_inner_inv_core s0 patch patch_n src src_n out
          src_seg_off src_seg_len tgt_len
          data_end inst_end addr_end src_seg dst0
          data_cursor inst_cursor addr_cursor tgt_pos np which s"
      and pop:
        "pop_byte (ds_inst_rem dst0) = Some (op, op_rest)"
      and step:
        "decode_one src_seg (unat src_seg_len) tgt_len dst0 = Inl dst_final"
      and tag_nz:
        "UCAST(8 \<rightarrow> 32)
          (code_tbl_'' s.[unat op].[unat (which * (3 :: 32 word))])
         \<noteq> (0 :: 32 word)"
      ...
      and size0:
        "UCAST(8 \<rightarrow> 32)
         ...
         = (0 :: 32 word)"
      and varint:
        "varint_decode (drop (unat inst_cursor)
          (heap_bytes s patch (unat inst_end))) = Some (sz, inst_rest)"
      and cache_abs_c: "cache_abs s c np"
      and cache_wf_c: "cache_wf c"
  shows "\<exists>addr arest c_next.
          decode_address c
            (unat
              (UCAST(8 \<rightarrow> 32)
                (code_tbl_'' s.[unat op]
                  .[unat (which * (3 :: 32 word) + 2)])
               :: 32 word))
            (unat (src_seg_len + tgt_pos))
            (drop (unat addr_cursor) (heap_bytes s patch (unat addr_end))) =
          Some (addr, arest, c_next) \<and>
          addr < unat (src_seg_len + tgt_pos) \<and>
          addr < 2 ^ 32"
proof -
  obtain dst_cur c_cur where core_cur:
    "decode_loop_inv_core s0 patch patch_n src src_n out
      src_seg_off src_seg_len tgt_len
      data_end inst_end addr_end src_seg
      data_cursor inst_cursor addr_cursor tgt_pos np dst_cur c_cur s"
    and prefix_cur:
    "decode_one_prefix src_seg (unat src_seg_len) tgt_len
      dst0 (unat which) dst_cur"
    using inner_inv
    unfolding decode_inner_inv_core_def by blast
  have code_tbl_cur: "code_tbl_matches s"
    using core_cur unfolding decode_loop_inv_core_def by simp
  have code_tbl_tags_cur: "code_tbl_tags_valid s"
    using core_cur unfolding decode_loop_inv_core_def by simp
  ...
  have h_size0: "isz h = 0"
    using code_tbl_current_half_varint_nonnoop(1)
      [OF code_tbl_cur code_tbl_tags_cur op_lt which_lt tag_nz size0]
      h_def
    by simp
  have h_copy:
    "ity h =
    ...
    using code_tbl_current_half_tag_three_copy
      [OF code_tbl_cur code_tbl_tags_cur op_lt which_lt
          tag_nz tag_ne1 tag_ne2]
      h_def
    by simp
  have inst_rem_cur:
    "ds_inst_rem dst_cur =
     drop (unat inst_cursor) (heap_bytes s patch (unat inst_end))"
    by (rule decode_loop_inv_core_inst_rem_current[OF core_cur])
  have vd_cur:
    "varint_decode (ds_inst_rem dst_cur) = Some (sz, inst_rest)"
    using varint inst_rem_cur by simp
  have resolve_cur_h:
    "resolve_size h (ds_inst_rem dst_cur) = Some (sz, inst_rest)"
  ...
  obtain st_exec where exec_cur:
    "exec_half
       ...
       sz src_seg (unat src_seg_len) tgt_len
       (dst_cur\<lparr>ds_inst_rem := inst_rest\<rparr>) =
     Inl st_exec"
    using decode_one_prefix_exec_half_some
      [OF pop prefix_cur step which_lt resolve_cur]
    by blast
  have exec_cur_h:
    "exec_half h sz src_seg (unat src_seg_len) tgt_len
       (dst_cur\<lparr>ds_inst_rem := inst_rest\<rparr>) =
     Inl st_exec"
    using exec_cur h_def by simp
  obtain addr arest c_next where dec_abs:
    "decode_address
       (ds_cache (dst_cur\<lparr>ds_inst_rem := inst_rest\<rparr>))
       (unat
         (UCAST(8 \<rightarrow> 32)
           (code_tbl_'' s.[unat op]
             .[unat (which * (3 :: 32 word) + 2)])
          :: 32 word))
       (unat src_seg_len +
         length (ds_tgt (dst_cur\<lparr>ds_inst_rem := inst_rest\<rparr>)))
       (ds_addr_rem (dst_cur\<lparr>ds_inst_rem := inst_rest\<rparr>)) =
    ...
    by blast
  have cache_abs_cur: "cache_abs s c_cur np"
    using core_cur unfolding decode_loop_inv_core_def by simp
  ...
  have tgt_cur:
    "length (ds_tgt dst_cur) = unat tgt_pos"
    using core_cur unfolding decode_loop_inv_core_def by simp
  have cache_cur:
    "ds_cache dst_cur = c_cur"
    using core_cur unfolding decode_loop_inv_core_def by simp
  have src_tgt_fit:
    "unat src_seg_len + tgt_len < 2 ^ 32"
    using core_cur unfolding decode_loop_inv_core_def by simp
  have tgt_pos_le: "unat tgt_pos \<le> tgt_len"
    using core_cur unfolding decode_loop_inv_core_def by simp
  have here_no_overflow:
    "unat src_seg_len + unat tgt_pos < 2 ^ 32"
    using src_tgt_fit tgt_pos_le by simp
  have here_eq:
    "unat (src_seg_len + tgt_pos :: 32 word) =
     unat src_seg_len + unat tgt_pos"
  ...
  have dec_c:
    "decode_address c
       ...
       (drop (unat addr_cursor) (heap_bytes s patch (unat addr_end))) =
     Some (addr, arest, c_next)"
    using dec_abs c_eq cache_cur addr_rem_cur tgt_cur here_eq by simp
  have addr_lt32: "addr < 2 ^ 32"
    using addr_bound tgt_cur here_no_overflow by simp
  have addr_lt_here: "addr < unat (src_seg_len + tgt_pos)"
    using addr_bound tgt_cur here_eq by simp
  show ?thesis
    using dec_c addr_lt_here addr_lt32 by blast
qed
...
proof (cases "decode_spec (heap_bytes s patch (unat patch_len))
  ...
  obtain win tail where
    ...
    by (auto split: sum.splits)
	  ...
	  have win_src_tgt_fit:
	    "pw_src_seg_len win + pw_tgt_len win < 2 ^ 32"
	    using source_target_window_fits[OF ph pw] .
        ...
        [where patch = patch and patch_n = "unat patch_len"
         ...
         runs_to_vcg]
	      supply runs_to_condition_exn [runs_to_vcg]
		      supply decode_address'_spec_ex [runs_to_vcg]
	      supply add_loop_correct_core_ex
	        [where s0 = td and patch = patch and patch_n = "unat patch_len"
	           and src = src and src_n = "unat src_len" and out = out
	           and src_seg_off = "val_C vaaa" and src_seg_len = "val_C vaa"
	           and tgt_len = "length tgt",
	         runs_to_vcg]
	      supply run_loop_correct_core_ex
	        [where s0 = td and patch = patch and patch_n = "unat patch_len"
	           and src = src and src_n = "unat src_len" and out = out
	           and src_seg_off = "val_C vaaa" and src_seg_len = "val_C vaa"
	           and tgt_len = "length tgt",
		           ...
		           qed
		             ...
		             show ?thesis
		               ...
		               proof -
		                 ...
		                 have dec6_td:
		                   ...
		                   using dec6 drop_rest6 by simp
			                 show ?thesis using prems q dec6_td by simp
		               qed
			               subgoal premises q for vab
		               proof -
		                 have pos_vab:
		               ...
		               proof -
		                 ...
		                 have dec7_td:
		                   ...
		                   using dec7 drop_rest7 by simp
				                 show ?thesis using prems q dec7_td by simp
			               qed
			               subgoal premises q for vab vaba
		               proof -
		                 have pos_vab:
		               ...
		               proof -
		                 ...
		                 qed
			                 have no_short: "\<not> val_C vaaaa < pr_t_C.pos_C vabaa - pr_t_C.pos_C vaaaa"
			                 proof -
			                   have "length rest3 - length rest8 \<le> unat (val_C vaaaa)"
				                 ...
				                 qed
				                 have addr_ok: "pr_t_C.err_C vabaa = 0"
				                   using q dec7_td by simp
				                 show ?thesis using q no_short addr_ok by simp
			               qed
			               subgoal premises q for vab vaba
			               proof -
			                 have pos_vaaaa:
					               ...
					               proof -
					                 ...
					                 proof -
					                   ...
					                   thus ?thesis
					                     using diff_unat by (simp add: word_less_nat_alt)
					                 qed
					                 show ?thesis using q no_short by simp
					               qed
					               subgoal premises q for vab vaba vabaa
					               proof -
					                 have pos_vaaaa:
					                 ...
					                 thus ?thesis using q by simp
					               qed
				               subgoal premises loop_prems for vab vaba vabaa
					                 apply (rule runs_to_weaken)
					                 apply (rule_tac base=td and patch=patch and patch_n="unat patch_len"
								                      and src=src and src_n="unat src_len" and out=out
								                      and tgt_len="length tgt"
							                      and tgt=tgt
							                      and src_seg=pw_src_seg
							                      and src_seg_off="val_C vaaa" and src_seg_len="val_C vaa"
							                      and data_end="pr_t_C.pos_C vabaa +
						                        (if UCAST(8 \<rightarrow> 32)
						                              (heap_w8 td (patch +\<^sub>p uint (pr_t_C.pos_C va + val_C va))) AND
					                            (4 :: 32 word) \<noteq> 0
					                         then 4 else 0) + val_C vab"
					                      and addr_end="pr_t_C.pos_C vabaa +
					                        (if UCAST(8 \<rightarrow> 32)
					                              (heap_w8 td (patch +\<^sub>p uint (pr_t_C.pos_C va + val_C va))) AND
					                            (4 :: 32 word) \<noteq> 0
						                         then 4 else 0) + val_C vab + val_C vaba + val_C vabaa"
						                 ...
						                 proof -
						                   let ?win_pos = "pr_t_C.pos_C va + val_C va"
							                   let ?alen_w =
							                     "((if UCAST(8 \<rightarrow> 32)
							                         ...
							                         (4 :: 32 word) \<noteq> 0
							                        then 4 else 0) :: 32 word)"
							                   let ?data_pos = "pr_t_C.pos_C vabaa + ?alen_w"
						                   let ?data_end = "?data_pos + val_C vab"
						                   ...
						                   have vals:
						                     ...
						                     apply (rule app_source_values
						                       [where td = td and va = va and vaa = vaa and vaaa = vaaa])
						                     using prems q apply simp_all
						                     done
						                   have src_seg_bound:
						                     "unat (val_C vaaa) + unat (val_C vaa) \<le> unat src_len"
						                     using vals win_src_bounds by arith
							                   have src_seg_bound32:
							                     "unat (val_C vaaa) + unat (val_C vaa) < 2 ^ 32"
							                   proof -
							                     have "unat (val_C vaaa) + unat (val_C vaa) \<le> unat src_len"
							                       using src_seg_bound .
							                     also have "\<dots> < 2 ^ 32"
							                   ...
							                   have src_tgt_len_fits:
							                     "unat (val_C vaa) + length tgt < 2 ^ 32"
							                     using vals win_src_tgt_fit tgt_len_eq by simp
						                   have pos_vab:
						                     "unat (pr_t_C.pos_C vab) = unat patch_len - length rest6"
						                     using loop_prems q dec5_td by simp
						                   have drop_rest6:
						                     "drop (unat (pr_t_C.pos_C vab)) ?bs_td = rest6"
						                     using varint_decode_drop_rest[OF dec5_td] pos_vab by simp
						                   have dec6_td:
						                   ...
						                   have pos_vaba:
						                     "unat (pr_t_C.pos_C vaba) = unat patch_len - length rest7"
						                     using loop_prems q dec6_td by simp
						                   have drop_rest7:
						                     "drop (unat (pr_t_C.pos_C vaba)) ?bs_td = rest7"
						                     using varint_decode_drop_rest[OF dec6_td] pos_vaba by simp
						                   have dec7_td:
						                   ...
						                   have pos_vabaa:
						                     "unat (pr_t_C.pos_C vabaa) = unat patch_len - length rest8"
						                     using loop_prems q dec7_td by simp
						                   have drop_rest8:
						                     "drop (unat (pr_t_C.pos_C vabaa)) ?bs_td = rest8"
						                     using varint_decode_drop_rest[OF dec7_td] pos_vabaa by simp
						                   have rest3_le_patch: "length rest3 \<le> unat patch_len"
						                   proof -
						                     have "length rest3 = length (drop (unat (pr_t_C.pos_C vaaaa)) ?bs_td)"
						                       using drop_rest3 by simp
						                   ...
						                   have inst_val: "unat (val_C vaba) = inst_len"
						                     using loop_prems q dec6_td by simp
							                   have addr_val: "unat (val_C vabaa) = addr_len"
							                     using loop_prems q dec7_td by simp
							                   have alen_unat: "unat ?alen_w = alen"
							                   proof (cases "UCAST(8 \<rightarrow> 32)
							                       (heap_w8 td
							                         (patch +\<^sub>p uint (pr_t_C.pos_C va + val_C va))) AND
							                      (4 :: 32 word) \<noteq> 0")
							                     case True
							                     have u4: "unat ?alen_w = 4"
							                       apply (subst if_P[OF True])
							                       apply simp
							                       done
							                     have bit_true:
							                       "heap_w8 td
							                         (patch +\<^sub>p uint (pr_t_C.pos_C va + val_C va)) !! 2"
							                       using True by word_bitwise
							                     have bit_true_eq:
							                       "(heap_w8 td
							                         (patch +\<^sub>p uint (pr_t_C.pos_C va + val_C va)) !! 2) = True"
							                       using bit_true by simp
							                     have bit_if_true:
							                       "(if heap_w8 td
							                           (patch +\<^sub>p uint (pr_t_C.pos_C va + val_C va)) !! 2
							                         then (4 :: nat) else 0) = 4"
							                     ...
							                     have a4: "alen = 4"
							                       ...
							                       by (cases "UCAST(8 \<rightarrow> 32)
							                         (heap_w8 td
							                           (patch +\<^sub>p uint (pr_t_C.pos_C va + val_C va))) AND
							                        (4 :: 32 word) \<noteq> 0") auto
							                     show ?thesis using u4 a4 by simp
							                   next
							                     case False
							                     have u0: "unat ?alen_w = 0"
							                       apply (subst if_not_P[OF False])
							                       apply simp
							                       done
							                     have bit_false:
							                       "\<not> heap_w8 td
							                         (patch +\<^sub>p uint (pr_t_C.pos_C va + val_C va)) !! 2"
							                       using False by word_bitwise
							                     have bit_false_eq:
							                       "(heap_w8 td
							                         (patch +\<^sub>p uint (pr_t_C.pos_C va + val_C va)) !! 2) = False"
							                       using bit_false by simp
							                     have bit_if_false:
							                       "(if heap_w8 td
							                           (patch +\<^sub>p uint (pr_t_C.pos_C va + val_C va)) !! 2
							                         then (4 :: nat) else 0) = 0"
							                     ...
							                     have a0: "alen = 0"
							                       ...
							                       by (cases "UCAST(8 \<rightarrow> 32)
							                         (heap_w8 td
							                           (patch +\<^sub>p uint (pr_t_C.pos_C va + val_C va))) AND
							                        (4 :: 32 word) \<noteq> 0") auto
							                     show ?thesis using u0 a0 by simp
							                   qed
						                   ...
						                   proof -
						                     have add_lt:
						                       "unat (pr_t_C.pos_C vabaa) + unat ?alen_w < 2 ^ 32"
						                     proof -
						                       have "unat (pr_t_C.pos_C vabaa) + unat ?alen_w \<le> unat patch_len"
						                         using pos_vabaa alen_unat sizes_ok rest8_le_patch by arith
						                       thus ?thesis using patch_lt by arith
						                     qed
						                     have "unat ?data_pos =
						                           unat (pr_t_C.pos_C vabaa) + unat ?alen_w"
						                       using add_lt by (simp add: unat_add_lem[THEN iffD1])
						                     also have "\<dots> = unat patch_len - length rest8 + alen"
						                       using pos_vabaa alen_unat rest8_le_patch by arith
						                     finally show ?thesis .
						                   qed
						                   have data_end_unat:
						                     "unat ?data_end =
						                   ...
						                   proof -
						                     have add_lt: "unat ?data_pos + unat (val_C vab) < 2 ^ 32"
						                     proof -
						                       have "unat ?data_pos + unat (val_C vab) \<le> unat patch_len"
						                         using data_pos_unat data_val sizes_ok rest8_le_patch by arith
						                       thus ?thesis using patch_lt by arith
						                     qed
						                     have "unat ?data_end = unat ?data_pos + unat (val_C vab)"
						                       using add_lt by (simp add: unat_add_lem[THEN iffD1])
						                     also have "\<dots> = unat patch_len - length rest8 + alen + data_len"
						                   ...
						                   proof -
						                     have add_lt: "unat ?data_end + unat (val_C vaba) < 2 ^ 32"
						                     proof -
						                       have "unat ?data_end + unat (val_C vaba) \<le> unat patch_len"
						                         using data_end_unat inst_val sizes_ok rest8_le_patch by arith
						                       thus ?thesis using patch_lt by arith
						                     qed
						                     have "unat ?inst_end = unat ?data_end + unat (val_C vaba)"
						                       using add_lt by (simp add: unat_add_lem[THEN iffD1])
						                     also have "\<dots> =
						                       unat patch_len - length rest8 + alen + data_len + inst_len"
						                   ...
						                   proof -
						                     have add_lt: "unat ?inst_end + unat (val_C vabaa) < 2 ^ 32"
						                     proof -
						                       have "unat ?inst_end + unat (val_C vabaa) \<le> unat patch_len"
						                         using inst_end_unat addr_val sizes_ok rest8_le_patch by arith
						                       thus ?thesis using patch_lt by arith
						                     qed
						                     have "unat ?addr_end = unat ?inst_end + unat (val_C vabaa)"
						                       using add_lt by (simp add: unat_add_lem[THEN iffD1])
						                     also have "\<dots> =
						                       unat patch_len - length rest8 + alen + data_len + inst_len + addr_len"
						                   ...
						                   have tgt_len_fits: "length tgt < 2 ^ 32"
						                     using tgt_val tgt_len_eq unat_lt2p[of "val_C vaaaaa"] by simp
						                   have src_seg_eq:
						                     "pw_src_seg =
						                      heap_bytes td (src +\<^sub>p uint (val_C vaaa)) (unat (val_C vaa))"
						                   proof -
						                     have view:
						                       "(if val_C vaa = 0 then []
						                         else take (unat (val_C vaa))
						                           (drop (unat (val_C vaaa))
						                             (heap_bytes td src (unat src_len)))) =
						                        heap_bytes td (src +\<^sub>p uint (val_C vaaa)) (unat (val_C vaa))"
						                       using heap_bytes_src_segment_view
						                         [OF src_seg_bound, of td src] .
						                     show ?thesis
						                     proof -
						                       have len_eq: "unat (val_C vaa) = pw_src_seg_len win"
						                         using vals by simp
						                       have off_eq: "unat (val_C vaaa) = pw_src_seg_off win"
						                         using vals by simp
						                       have zero_eq: "(val_C vaa = 0) = (pw_src_seg_len win = 0)"
						                       proof
						                         assume "val_C vaa = 0"
						                         thus "pw_src_seg_len win = 0"
						                           using len_eq by simp
						                       next
						                         assume "pw_src_seg_len win = 0"
						                         hence "unat (val_C vaa) = 0"
						                           using len_eq by simp
						                         thus "val_C vaa = 0"
						                           using word_unat_eq_iff[of "val_C vaa" 0] by simp
						                       qed
						                       have src_heap_eq:
						                         "heap_bytes td src (unat src_len) =
						                          heap_bytes s src (unat src_len)"
						                         using prems q by simp
						                       have view':
						                         "(if pw_src_seg_len win = 0 then []
						                           else take (pw_src_seg_len win)
						                             (drop (pw_src_seg_off win)
						                               (heap_bytes td src (unat src_len)))) =
						                          heap_bytes td (src +\<^sub>p uint (val_C vaaa))
						                            (pw_src_seg_len win)"
						                       proof (cases "pw_src_seg_len win = 0")
						                         case True
						                         hence "val_C vaa = 0"
						                           using zero_eq by simp
						                         with True show ?thesis
						                           using view len_eq by simp
						                       next
						                         case False
						                         hence "val_C vaa \<noteq> 0"
						                           using zero_eq by simp
						                         with False show ?thesis
						                           using view len_eq off_eq by simp
						                       qed
						                       have src_left_eq:
						                         "(if pw_src_seg_len win = 0 then []
						                           else take (pw_src_seg_len win)
						                       ...
						                       next
						                         case False
						                         have src_heap_eq_sym:
						                           "heap_bytes s src (unat src_len) =
						                            heap_bytes td src (unat src_len)"
						                           using src_heap_eq by (rule sym)
						                         show ?thesis
						                           using False by (simp add: src_heap_eq_sym)
						                       qed
						                       show ?thesis
						                         using src_left_eq view' len_eq
						                       unfolding pw_src_seg_def
						                       by simp
						                     qed
						                   qed
						                   have win_pos_lt: "?win_pos < patch_len"
						                     using prems loop_prems q by simp
						                   have win_pos_suc:
						                     "unat (?win_pos + 1) = Suc (unat ?win_pos)"
						                     using unat_x_plus_1[OF win_pos_lt] by simp
						                   have hi4: "hi AND (4 :: 8 word) \<noteq> 0"
						                     using prems hi_v ucast_and_4_equiv by simp
						                   obtain app_len app_rest where app_dec:
						                     "varint_decode body = Some (app_len, app_rest)"
						                     and rest_eq: "rest = drop app_len app_rest"
						                     using parse_header_app[unfolded app_bit_def] hi4 by blast
						                   have app_dec_bs:
						                     "varint_decode (drop 5 (heap_bytes s patch (unat patch_len))) =
						                      Some (app_len, app_rest)"
						                     using app_dec body_from_drop5 by simp
						                   have app_len_eq: "app_len = unat (val_C va)"
						                     and app_pos_eq:
						                       "unat (pr_t_C.pos_C va) = unat patch_len - length app_rest"
						                     using prems app_dec_bs by auto
						                   have app_pos_drop:
						                     "drop (unat (pr_t_C.pos_C va))
						                       (heap_bytes s patch (unat patch_len)) = app_rest"
						                     using varint_decode_drop_rest[OF app_dec_bs] app_pos_eq by simp
						                   have app_val_le: "val_C va \<le> patch_len - pr_t_C.pos_C va"
						                     using prems by (simp add: word_le_not_less)
						                   have win_pos_unat:
						                     "unat ?win_pos =
						                      unat (pr_t_C.pos_C va) + unat (val_C va)"
						                   proof -
						                     have pos_le: "pr_t_C.pos_C va \<le> patch_len"
						                       using prems q by simp
						                     have sum_le:
						                       "unat (pr_t_C.pos_C va) + unat (val_C va) \<le> unat patch_len"
						                     proof -
						                       have "unat (patch_len - pr_t_C.pos_C va) =
						                             unat patch_len - unat (pr_t_C.pos_C va)"
						                         using pos_le by (simp add: unat_sub word_le_nat_alt)
						                       hence "unat (val_C va) \<le>
						                              unat patch_len - unat (pr_t_C.pos_C va)"
						                         using app_val_le by (simp add: word_le_nat_alt)
						                       thus ?thesis
						                         using pos_le by (simp add: word_le_nat_alt)
						                     qed
						                     have sum_lt:
						                       "unat (pr_t_C.pos_C va) + unat (val_C va) < 2 ^ 32"
						                       using sum_le unat_lt2p[of patch_len] by simp
						                     show ?thesis
						                       using sum_lt by (simp add: unat_add_lem[THEN iffD1])
						                   qed
						                   have rest_at_win_pos_s:
						                     "drop (unat ?win_pos)
						                   ...
						                   proof -
						                     ...
						                     also have "\<dots> = drop (unat (val_C va)) app_rest"
						                       using app_pos_drop by simp
						                     also have "\<dots> = rest"
						                       using rest_eq app_len_eq by simp
						                     finally show ?thesis .
						                   qed
						                   have rest_at_win_pos_td:
						                     "drop (unat ?win_pos) ?bs_td = rest"
						                   proof -
						                     have patch_heap_eq:
						                       "?bs_td = heap_bytes s patch (unat patch_len)"
						                       using prems q by simp
						                     show ?thesis
						                       using rest_at_win_pos_s by (simp add: patch_heap_eq)
						                   qed
						                   obtain rest1 rest2 dlen0 rest30 where dec1:
						                     "varint_decode
						                       (drop (unat (?win_pos + 1)) ?bs_td) =
						                      Some (pw_src_seg_len win, rest1)"
						                     and dec2:
						                       "varint_decode rest1 = Some (pw_src_seg_off win, rest2)"
						                     and dec3_prefix:
						                       "varint_decode rest2 = Some (dlen0, rest30)"
						                     using app_source_prefix_decodes[where td=td and va=va] prems q
						                     by blast
						                   have pos_vaa:
						                     "unat (pr_t_C.pos_C vaa) = unat patch_len - length rest1"
						                     using prems loop_prems q dec1 by (auto split: option.splits)
						                   have drop_rest1:
						                     "drop (unat (pr_t_C.pos_C vaa)) ?bs_td = rest1"
						                     using varint_decode_drop_rest[OF dec1] pos_vaa by simp
						                   have dec2_td:
						                     "varint_decode
						                       (drop (unat (pr_t_C.pos_C vaa)) ?bs_td) =
						                      Some (pw_src_seg_off win, rest2)"
						                     using dec2 drop_rest1 by simp
						                   have pos_vaaa:
						                     "unat (pr_t_C.pos_C vaaa) = unat patch_len - length rest2"
						                     using prems loop_prems q dec2_td by (auto split: option.splits)
						                   have drop_rest2:
						                     "drop (unat (pr_t_C.pos_C vaaa)) ?bs_td = rest2"
						                     using varint_decode_drop_rest[OF dec2_td] pos_vaaa by simp
						                   have dec3_rest2:
						                     "varint_decode rest2 = Some (dlen, rest3)"
						                     using dec3 drop_rest2 by simp
						                   have win_ind_nth:
						                     ...
						                     using win_pos_lt by (simp add: heap_bytes_nth word_less_nat_alt)
						                   have pop_win:
						                   ...
						                   proof -
						                     ...
						                     proof -
						                       have idx_lt: "unat ?win_pos < length ?bs_td"
						                         using win_pos_lt by (simp add: word_less_nat_alt)
						                       have drop_cons:
						                         "?bs_td ! unat ?win_pos #
						                          drop (Suc (unat ?win_pos)) ?bs_td =
						                   ...
						                   proof -
						                     have no_target32:
						                       "UCAST(8 \<rightarrow> 32)
						                        ...
						                        (2 :: 32 word) = 0"
						                       using prems loop_prems q by simp
						                     show ?thesis using no_target32 by word_bitwise
						                   qed
						                   have mask_ok8:
						                     "heap_w8 td (patch +\<^sub>p uint ?win_pos) AND
						                      (0xFA :: 8 word) = 0"
						                   proof -
						                     have mask32:
						                       "UCAST(8 \<rightarrow> 32)
						                        ...
						                        (0xFFFFFFFA :: 32 word) = 0"
						                       using prems loop_prems q by simp
						                     show ?thesis using mask32 by word_bitwise
						                   qed
						                   have has_source8:
						                     "heap_w8 td (patch +\<^sub>p uint ?win_pos) AND
						                      (0x01 :: 8 word) \<noteq> 0"
						                   proof -
						                     have source32:
						                       "UCAST(8 \<rightarrow> 32)
						                        ...
						                        (1 :: 32 word) = 1"
						                       using prems loop_prems q by simp
						                     show ?thesis using source32 by word_bitwise
						                   qed
						                   have alen_eq8:
						                     "alen =
						                   ...
						                   have parsed_payload:
						                     "parse_window (drop (unat ?win_pos) ?bs_td) =
						                      Inl
						                       (\<lparr>pw_src_seg_len = pw_src_seg_len win,
						                         pw_src_seg_off = pw_src_seg_off win,
						                     ...
						                     by (rule parse_window_with_source_adler_general
						                       [OF pop_win no_target8 mask_ok8 has_source8 alen_eq8
						                           dec1 dec2 dec3_rest2 dec4 di0 refl dec5 dec6 dec7
						                           sizes_ok dlen_exact])
						                   have parsed_at_win:
						                     "parse_window (drop (unat ?win_pos) ?bs_td) = Inl (win, tail)"
						                     using rest_at_win_pos_td pw by simp
						                   have win_payload_eq:
						                     "win =
						                       \<lparr>pw_src_seg_len = pw_src_seg_len win,
						                         pw_src_seg_off = pw_src_seg_off win,
						                         ...
						                         pw_addr =
						                           take addr_len
						                             (drop (alen + data_len + inst_len) rest8)\<rparr>"
						                     using parsed_payload parsed_at_win by auto
						                   have pw_data_eq:
						                   ...
						                   have drop_base_rest8: "drop ?base ?bs_td = rest8"
						                     using pos_vabaa drop_rest8 by simp
						                     ...
						                     "\<exists>dst_final.
						                        decode_loop
						                          (length (drop (unat ?data_end)
						                            (take (unat ?inst_end) ?bs_td)))
						                          pw_src_seg (unat (val_C vaa)) (length tgt) ?dst0 =
						                          Inl dst_final \<and>
						                   ...
						                   qed
							                   have code_tbl_td: "code_tbl_matches td"
							                     using prems q by (simp add: code_tbl_matches_def)
							                   have code_tbl_tags_td: "code_tbl_tags_valid td"
							                     using code_tbl_ready code_tbl_tags_ready prems q
							                     by (simp add: code_tbl_tags_valid_def)
							                   have patch_valid_td: "buf_valid td patch (unat patch_len)"
						                     using patch_ok prems q by (simp add: buf_valid_def)
					                 ...
					                 apply runs_to_vcg
					                  ...
					                  subgoal
					                    ...
					                    proof -
					                      ...
					                      have tgt_val_win:
					                        "unat (val_C vaaaaa) = pw_tgt_len win"
					                        apply (rule app_source_tgt_value
					                          [where td=td and va=va and vaa=vaa
					                             and vaaa=vaaa and vaaaa=vaaaa
					                             and vaaaaa=vaaaaa])
					                        using prems loop_prems apply simp_all
					                        done
					                      have tgt_val_exit:
					                        "unat (val_C vaaaaa) = length tgt"
					                    ...
					                    obtain dst c dst' dst_final k where
					                      core_before:
					                        "decode_loop_inv_core td patch (unat patch_len)
					                          ...
					                          (val_C vaa) (length tgt)
					                          ?data_end ?inst_end ?addr_end
					                          pw_src_seg dc ic ac tp np dst c t"
					                      and decode_one_step:
					                        "decode_one pw_src_seg (unat (val_C vaa)) (length tgt) dst = Inl dst'"
					                      and decode_loop_rest:
					                        "decode_loop k pw_src_seg (unat (val_C vaa)) (length tgt) dst'
					                          = Inl dst_final"
					                        ...
					                        apply runs_to_vcg
					                         ...
					                         proof -
					                           ...
					                           have inner_inv:
					                             ...
					                             using exit_prems tuple_eq by (auto split: exception_or_result_splits)
					                           ...
					                           obtain dst_i c_i where core_i:
					                             "decode_loop_inv_core td patch (unat patch_len)
					                               ...
					                               (val_C vaa) (length tgt)
					                               ?data_end ?inst_end ?addr_end
					                               pw_src_seg dc_i ic_i ac_i tp_i np_i dst_i c_i st"
					                             and prefix_i:
					                             "decode_one_prefix pw_src_seg (unat (val_C vaa)) (length tgt)
					                               dst (unat which_i) dst_i"
					                             using inner_inv unfolding decode_inner_inv_core_def by blast
					                           have step_i:
					                             "decode_one pw_src_seg (unat (val_C vaa)) (length tgt) dst = Inl dst_i"
					                           ...
					                           have core_after:
					                             "decode_loop_inv_core td patch (unat patch_len)
					                               ...
					                               (val_C vaa) (length tgt)
					                               ?data_end ?inst_end ?addr_end
					                               pw_src_seg dc_i ic_i ac_i tp_i np_i dst' (ds_cache dst') st"
					                           proof -
					                           ...
					                           have core_before_ds:
					                             "decode_loop_inv_core td patch (unat patch_len)
					                               ...
					                               (val_C vaa) (length tgt)
					                               ?data_end ?inst_end ?addr_end
					                               pw_src_seg dc ic ac tp np dst (ds_cache dst) t"
					                             using core_before
					                             unfolding decode_loop_inv_core_def by simp
					                           have plus_after:
					                             "decode_loop_inv_plus td patch (unat patch_len)
					                               ...
					                               (val_C vaa) (length tgt)
					                               ?data_end ?inst_end ?addr_end
					                               pw_src_seg tgt dc_i ic_i ac_i tp_i np_i st"
						                           ...
						                           proof -
						                             ...
						                             have inv_cur:
						                               "decode_loop_inv td patch (unat patch_len)
						                                 ...
						                                 (val_C vaa) (length tgt)
						                                 ?data_end ?inst_end ?addr_end
						                                 pw_src_seg x1a x1b x1 x1d x1c s_inner"
							                             ...
							                             have vd_heap:
								                           ...
								                           proof -
								                             ...
								                             obtain st_exec where exec_cur:
								                               ...
								                               by blast
								                             have exec_cur_h:
								                               "exec_half h (unat (val_C vac)) pw_src_seg
								                                  (unat (val_C vaa)) (length tgt)
								                                  (dst_cur\<lparr>ds_inst_rem := rest\<rparr>) =
								                                Inl st_exec"
								                               using exec_cur h_def by simp
								                           ...
								                           qed
									                           ...
									                           proof -
									                             ...
									                             have heap_inst_eq:
									                               "heap_bytes s_inner patch (unat ?inst_end) =
									                                heap_bytes td patch (unat ?inst_end)"
									                             proof (rule heap_bytes_eqI)
									                               fix i
									                             ...
									                             have inv_after_varint:
									                               "decode_loop_inv td patch (unat patch_len)
									                                 ...
									                                 (val_C vaa) (length tgt)
									                                 ?data_end ?inst_end ?addr_end
									                                 pw_src_seg x1a (pr_t_C.pos_C vac) x1 x1d x1c s_inner"
									                             ...
									                             show ?thesis
									                               ...
									                               proof -
									                                 ...
									                                 proof (intro allI impI)
									                                   ...
									                                   also have "... = heap_w8 td (patch +\<^sub>p int j)"
									                                 ...
									                                 proof (intro allI impI)
									                                   ...
									                                   have "heap_w8 t' (src +\<^sub>p int j) =
									                                     ...
									                                     using loop_post j_lt by simp
									                                   also have "... = heap_w8 td (src +\<^sub>p int j)"
									                                     using core_after_varint j_lt
									                                     unfolding decode_loop_inv_core_def by simp
									                                   finally show
									                                     "heap_w8 t' (src +\<^sub>p int j) =
									                                   ...
									                                   proof -
									                                     ...
									                                     also have "... =
									                                           heap_w8 td
									                                             (patch +\<^sub>p int
									                                               (unat (x1a + of_nat j :: 32 word)))"
									                                       using patch_sinner_td[rule_format, OF idx_lt_patch] by simp
									                                     also have "... =
									                                           heap_w8 td
									                                             (patch +\<^sub>p uint (x1a + of_nat j))"
									                                       by (simp only: uint_nat)
									                                 ...
									                                 have core_after_add:
									                                   "decode_loop_inv_core td patch (unat patch_len)
									                                     ...
									                                     (val_C vaa) (length tgt)
									                                     ?data_end ?inst_end ?addr_end
									                                     pw_src_seg (x1a + val_C vac)
									                                     (pr_t_C.pos_C vac) x1
									                                     (x1d + val_C vac) x1c
									                                     st_exec c_cur t'"
									                                   by (rule decode_loop_inv_core_after_add_exec
									                                     [OF core_after_varint h_add exec_cur_h data_fit tgt_fit
									                                         no_overflow_data no_overflow_tgt patch_t_td src_t_td
									                                         add_result_td out_prefix typing_preserved
									                           ...
									                           proof -
									                             ...
									                             have fill_eq_td:
									                               "heap_w8 s_inner (patch +\<^sub>p uint x1a) =
									                                heap_w8 td (patch +\<^sub>p int (unat x1a))"
									                             proof -
									                               ...
									                               also have "... =
									                                     heap_w8 td (patch +\<^sub>p int (unat x1a))"
									                                 using patch_sinner_td[rule_format, OF x1a_bound] by simp
									                               finally show ?thesis .
									                             qed
									                             have prefix_next:
									                               "decode_one_prefix pw_src_seg (unat (val_C vaa)) (length tgt)
									                               ...
									                               proof -
									                                 ...
									                                 proof (intro allI impI)
									                                   ...
									                                   have "heap_w8 t' (patch +\<^sub>p int j) =
									                                     ...
									                                     using loop_post j_lt by simp
									                                   also have "... = heap_w8 td (patch +\<^sub>p int j)"
									                                     using patch_sinner_td[rule_format, OF j_lt] by simp
									                                   finally show
									                                     "heap_w8 t' (patch +\<^sub>p int j) =
									                                      heap_w8 td (patch +\<^sub>p int j)" .
									                                 qed
									                                 ...
									                                 have core_after_run:
									                                   ...
									                                   by (rule decode_loop_inv_core_after_run_exec
									                                     [OF core_after_varint h_run exec_cur_h data_lt tgt_fit
									                                         no_overflow_tgt no_overflow_data patch_t_td src_t_td
									                                         run_result fill_eq_td out_prefix typing_preserved
									                                         near_preserved same_preserved code_preserved])
									                                 show ?thesis
									                                 proof (cases r)
									                                   case (Exception e)
									                           ...
									                           proof -
									                             ...
									                             obtain dst_cur c_cur where core_cur:
										                           ...
										                           subgoal premises copy_addr_err_prems for vac ta c ar
										                           proof -
										                             have which_lt: "unat x2d < 2"
										                             ...
										                             have tgt_pos_le: "unat x1d \<le> length tgt"
										                               using core_cur unfolding decode_loop_inv_core_def by simp
										                             have here_no_overflow:
										                               "unat (val_C vaa) + unat x1d < 2 ^ 32"
										                               using src_tgt_fit tgt_pos_le by simp
										                             have here_eq:
										                               "unat (val_C vaa + x1d :: 32 word) =
										                                unat (val_C vaa) + unat x1d"
										                               using here_no_overflow
										                               by (simp add: unat_word_ariths(1))
										                             have dec_c:
										                               "decode_address c
										                           ...
										                           qed
										                           subgoal premises copy_addr_invalid_prems for vac ta c ar
										                           proof -
										                             have which_lt: "unat x2d < 2"
										                             ...
										                             have addr_unat:
										                               ...
										                               by (simp add: unat_of_nat_eq)
											                             show ?thesis
											                               ...
											                               by simp
											                           qed
											                           subgoal premises copy_core_prems for vac ta c ar
											                           proof -
											                             have which_lt: "unat x2d < 2"
											                             ...
											                             have np_bound:
											                               ...
											                               by (simp split: option.splits prod.splits if_splits)
											                             have cache_abs_ta:
											                               "cache_abs ta c_next (near_ptr_C ar)"
											                               using copy_core_prems dec_c addr_lt32
											                               by (simp split: option.splits prod.splits if_splits)
											                             ...
											                             have irest_align:
											                               "irest =
											                                drop (unat (pr_t_C.pos_C vac))
											                                  (take (unat ?inst_end)
											                                    (heap_bytes td patch (unat patch_len)))"
											                               using irest_sinner heap_inst_eq take_inst_td by simp
											                             have core_after_varint:
											                               "decode_loop_inv_core td patch (unat patch_len)
											                                 src (unat src_len) out (val_C vaaa)
											                                 (val_C vaa) (length tgt)
											                             ...
											                             have heap_addr_eq:
											                               "heap_bytes s_inner patch (unat ?addr_end) =
											                                heap_bytes td patch (unat ?addr_end)"
											                             proof (rule heap_bytes_eqI)
											                               fix i
											                             ...
											                             have arest_align:
											                               "arest =
											                                drop (unat (ar_t_C.pos_C ar))
											                                  (take (unat ?addr_end)
											                                    (heap_bytes td patch (unat patch_len)))"
											                               using arest_sinner heap_addr_eq take_addr_td by simp
											                             ...
											                             have heap_ta: "heap_w8 ta = heap_w8 s_inner"
											                               using copy_core_prems by simp
											                             have typing_ta: "heap_typing ta = heap_typing s_inner"
											                               using copy_core_prems by simp
											                             have code_ta: "code_tbl_'' ta = code_tbl_'' s_inner"
											                               using copy_core_prems by simp
											                             ...
											                             have inv_after_addr:
											                               "decode_loop_inv td patch (unat patch_len)
											                                 ...
											                                 ?data_end ?inst_end ?addr_end
											                                 pw_src_seg x1a (pr_t_C.pos_C vac)
											                                 (ar_t_C.pos_C ar) x1d (near_ptr_C ar) ta"
											                             ...
											                             have addr_bound_word:
											                               "unat (addr_C ar) < unat (val_C vaa) + unat x1d"
											                             ...
											                             have copy_run:
											                               "(whileLoop (\<lambda>(j :: 32 word) st.
											                                  ...
											                                  (\<lambda>j. do {
											                                    byte \<leftarrow>
											                                      condition (\<lambda>st.
											                                        unat (addr_C ar + j) < unat (val_C vaa))
											                                        (liftE (do {
											                                          guard (\<lambda>st.
											                                            ptr_valid (heap_typing st)
											                                              (src +\<^sub>p uint (val_C vaaa + (addr_C ar + j))));
											                                          gets (\<lambda>st.
											                                            heap_w8 st
											                                              (src +\<^sub>p uint (val_C vaaa + (addr_C ar + j))))
											                                        }))
											                                        (do {
											                                          when
											                                            (unat (x1d + j) \<le>
											                                             unat (addr_C ar + j - val_C vaa))
											                                            (throw (- (10 :: int)));
											                                          liftE (do {
											                                            guard (\<lambda>st.
											                                              ptr_valid (heap_typing st)
											                                                (out +\<^sub>p uint (addr_C ar + j - val_C vaa)));
											                                            gets (\<lambda>st.
											                                              heap_w8 st
											                                                (out +\<^sub>p uint (addr_C ar + j - val_C vaa)))
											                                          })
											                                  ...
											                                  (int, 32 word, lifted_globals) exn_monad) \<bullet> ta
											                                \<lbrace> \<lambda>r t'.
											                                  r = Result (val_C vac) \<and>
											                                  (\<forall>j < unat patch_len.
											                                    heap_w8 t' (patch +\<^sub>p int j) =
											                                    heap_w8 ta (patch +\<^sub>p int j)) \<and>
											                                  (\<forall>j < unat src_len.
											                                    heap_w8 t' (src +\<^sub>p int j) =
											                                    heap_w8 ta (src +\<^sub>p int j)) \<and>
											                                  (\<forall>j < unat x1d.
											                                    heap_w8 t' (out +\<^sub>p int j) =
											                                    heap_w8 ta (out +\<^sub>p int j)) \<and>
											                                  (\<forall>k < unat (val_C vac).
											                                    heap_w8 t'
											                                      (out +\<^sub>p uint (x1d + of_nat k)) =
											                                    copy_loop pw_src_seg
											                                      (heap_bytes ta out (unat x1d))
											                                      (unat (addr_C ar)) (unat (val_C vac)) !
											                                      (unat x1d + k)) \<and>
											                                  heap_typing t' = heap_typing ta \<and>
											                                  near_arr_'' t' = near_arr_'' ta \<and>
											                                  same_arr_'' t' = same_arr_'' ta \<and>
											                                  code_tbl_'' t' = code_tbl_'' ta \<rbrace>"
											                               by (rule copy_loop_correct_core
											                             ...
											                             show ?thesis
											                               ...
											                               proof -
											                                 ...
											                                 proof (rule decode_loop_inv_core_after_copy_fields
											                                   ...
											                                   show "ds_tgt st_exec =
											                                     copy_loop pw_src_seg (ds_tgt ?dst_addr)
											                                       (unat (addr_C ar)) (unat (val_C vac))"
											                                   ...
											                                   show "\<forall>i<unat x1d.
											                                     heap_w8 t' (out +\<^sub>p int i) =
											                                     heap_w8 ta (out +\<^sub>p int i)"
											                                     using loop_post by simp
											                                   show "heap_typing t' = heap_typing ta"
											                                     using loop_post by simp
											                                   show "near_arr_'' t' = near_arr_'' ta"
											                                     using loop_post by simp
											                                   show "same_arr_'' t' = same_arr_'' ta"
											                                     using loop_post by simp
											                                   show "code_tbl_'' t' = code_tbl_'' ta"
											                                     using loop_post by simp
											                                 qed
											                                 show ?thesis
											                                 proof (cases r)
											                                 ...
											                                 next
											                                   ...
											                                   have inner_after:
											                                     "decode_inner_inv_core td patch (unat patch_len)
											                                       ...
											                                       (val_C vaa) (length tgt)
											                                       ?data_end ?inst_end ?addr_end
											                                       pw_src_seg dst x1a (pr_t_C.pos_C vac)
											                                       (ar_t_C.pos_C ar) (x1d + val_C vac)
												                           ...
												                           proof -
												                             ...
												                             have exec_cur_h_simp:
												                               "exec_half h (unat ?sz) pw_src_seg
												                                  (unat (val_C vaa)) (length tgt) dst_cur =
												                                Inl st_exec"
		               ...
		               using varint_decode_drop_rest[OF dec3] pos_vaa by simp
			             have dec4_td:
			               "varint_decode
			                ...
			                Some (pw_tgt_len win, rest4)"
			               using dec4 drop_rest3 by simp
			             have tgt_ok: "pr_t_C.err_C vaaa = 0"
		             ...
		             hence "unat (val_C vaaa) \<le> unat out_cap"
		               using tgt_val tgt_len_eq by simp
			             hence cap_ok: "val_C vaaa \<le> out_cap"
			               by (simp add: word_le_nat_alt)
				             show ?thesis
				               using prems tgt_ok cap_ok by simp
				           qed
				           subgoal premises prems for vaa vaaa
				           proof -
				             obtain rest3 rest4 where
